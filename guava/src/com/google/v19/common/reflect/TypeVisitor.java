...
/**
 * Based on what a {@link Type} is, dispatch it to the corresponding
 * {@code visit*} method. By default, no recursion is done for type arguments or
 * type bounds. But subclasses can opt to do recursion by calling {@link #visit}
 * for any {@code Type} while visitation is in progress. For example, this can
 * be used to reject wildcards or type variables contained in a type as in:
 *
 * <pre>
 *    {@code
 *   new TypeVisitor() {
 ...
 * 
 * <p>
 * One {@code Type} is visited at most once. The second time the same type is
...
abstract class TypeVisitor {
   ...
   * call {@code visit(parameterizedType.getOwnerType())} safely without having to
   * check nulls.
   */
