...
package com.google.v19.common.reflect;
 ...
 * Mappings are established with {@link #where} and types are resolved using
 * {@link #resolveType}.
 *
 * <p>
 * Note that usually type mappings are already implied by the static type
 * hierarchy (for example, the {@code E} type variable declared by class
 * {@code List} naturally maps to {@code String} in the context of
 * {@code class MyStringList implements List<String>}. In such case, prefer to
 * use {@link TypeToken#resolveType} since it's simpler and more type safe. This
 * class should only be used when the type mapping isn't implied by the static
 * type hierarchy, but provided through other means such as an annotation or
...
public final class TypeResolver {
  ...
  /**
   * Returns a new {@code TypeResolver} with type variables in {@code formal}
   * mapping to types in {@code actual}.
   *
   * <p>
   * For example, if {@code formal} is a {@code TypeVariable T}, and
   * {@code actual} is {@code
   ...
   * {@code Map<String, Something>} etc. Similarly, {@code formal} and
   * {@code actual} can be {@code Map<K, V>} and {@code Map<String, Integer>}
   * respectively, or they can be {@code E[]} and {@code String[]} respectively,
   * or even any arbitrary combination thereof.
   *
   * @param formal The type whose type variables or itself is mapped to other
   *               type(s). It's almost always a bug if {@code formal} isn't a
   *               type variable and contains no type variable. Make sure you are
   *               passing the two parameters in the right order.
   * @param actual The type that the formal type variable(s) are mapped to. It can
   *               be or contain yet other type variables, in which case these
   *               type variables will be further resolved if corresponding
   *               mappings exist in the current {@code TypeResolver} instance.
   */
  ...
  private static void populateTypeMappings(final Map<TypeVariableKey, Type> mappings, Type from, final Type to) {
    ...
    if (to instanceof WildcardType != from instanceof WildcardType) {
      // When we are saying "assuming <?> is T, there really isn't any useful type
      // mapping.
    ...
    new TypeVisitor() {
      ...
      void visitWildcardType(WildcardType fromWildcardType) {
        ...
        checkArgument(fromUpperBounds.length == toUpperBounds.length && fromLowerBounds.length == toLowerBounds.length,
  ...
  private static class TypeTable {
    ...
    Type resolveInternal(TypeVariable<?> var, TypeTable forDependants) {
      ...
      if (type == null) {
         ...
         * TypeVariables from the JDK against TypeVariables returned by TypeResolver. To
         * work with all JDK versions, TypeResolver must return the appropriate
         * TypeVariable implementation in each of the three possible cases:
         *
         ...
         * interoperate with ours. Therefore, we have to be careful about whether we
         * create our own TypeVariable:
         *
         * 2a. If the resolved types are identical to the original types, then we can
         * return the original, identical JDK TypeVariable. By doing so, we sidestep the
         * problem entirely.
         *
         * 2b. If the resolved types are different from the original types, things are
         * trickier. The only way to get a TypeVariable instance for the resolved types
         * is to create our own. The created TypeVariable will not interoperate with any
         * JDK TypeVariable. But this is OK: We don't _want_ our new TypeVariable to be
         * equal to the JDK TypeVariable because it has _different bounds_ than the JDK
         * TypeVariable. And it wouldn't make sense for our new TypeVariable to be equal
         * to any _other_ JDK TypeVariable, either, because any other JDK TypeVariable
         * must have a different declaration or name. The only TypeVariable that our new
         * TypeVariable _will_ be equal to is an equivalent TypeVariable that was also
         * created by us. And that equality is guaranteed to hold because it doesn't
         * involve the JDK TypeVariable implementation at all.
         */
  ...
  /**
   * Wraps around {@code TypeVariable<?>} to ensure that any two type variables
   * are equal as long as they are declared by the same
   * {@link java.lang.reflect.GenericDeclaration} and have the same name, even if
   * their bounds differ.
   *
   * <p>
   * While resolving a type variable from a {var -> type} map, we don't care
   * whether the type variable's bound has been partially resolved. As long as the
   * type variable "identity" matches.
   *
   * <p>
   ...
   * <A extends String> in order to decide to use the transformed type instead of
   * the original type.
   */
