...
/**
 * A map, each entry of which maps a {@link TypeToken} to an instance of that
 * type. In addition to implementing {@code Map}, the additional type-safe
 * operations {@link #putInstance} and {@link #getInstance} are available.
 *
 * <p>
 * Generally, implementations don't support {@link #put} and {@link #putAll}
 * because there is no way to check an object at runtime to be an instance of a
 * {@link TypeToken}. Instead, caller should use the type safe
 * {@link #putInstance}.
 * 
 * <p>
 * Also, if caller suppresses unchecked warnings and passes in an
 * {@code Iterable<String>} for type {@code Iterable<Integer>}, the map won't be
 * able to detect and throw type error.
 *
 * <p>
...
public interface TypeToInstanceMap<B> extends Map<TypeToken<? extends B>, B> {
   ...
   * 
   * <p>
   * {@code getInstance(Foo.class)} is equivalent to
   ...
   *
   * @return the value previously associated with this class (possibly
   ...
   * entry for this type is present. This will only return a value that was bound
   * to this specific type, not a value that may have been bound to a subtype.
   */
  @Nullable
   ...
   *
   * @return the value previously associated with this type (possibly
   *         {@code null}), or {@code null} if there was no previous entry.
   */
  @Nullable
