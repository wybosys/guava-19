...
/**
 * Wrapper around either a {@link Method} or a {@link Constructor}. Convenience
 * API is provided to make common reflective operation easier to deal with, such
 * as {@link #isPublic}, {@link #getParameters} etc.
 *
 * <p>
 * In addition to convenience methods, {@link TypeToken#method} and
 * {@link TypeToken#constructor} will resolve the type parameters of the method
 * or constructor in the context of the owner type, which may be a subtype of
 * the declaring class. For example:
 *
 * <pre>
 * {
 *   &#64;code
 ...
 * @param <R> the return type of (or supertype thereof) the method or the
...
public abstract class Invokable<T, R> extends Element implements GenericDeclaration {
   ...
   * static or final methods, or methods declared by final classes are not
   * overridable.
   */
   ...
   *
   * @throws IllegalAccessException    if this {@code Constructor} object enforces
   *                                   Java language access control and the
   *                                   underlying method or constructor is
   *                                   inaccessible.
   ...
   * @throws InvocationTargetException if the underlying method or constructor
   *                                   throws an exception.
   */
  ...
  /**
   * Returns all declared parameters of this {@code Invokable}. Note that if this
   * is a constructor of a non-static inner class, unlike
   * {@link Constructor#getParameterTypes}, the hidden {@code this} parameter of
   * the enclosing class is excluded from the returned parameters.
   */
  ...
  static class ConstructorInvokable<T> extends Invokable<T, T> {
     ...
     * {@inheritDoc}
     *
     * {@code [<E>]} will be returned for ArrayList's constructor. When both the
     * class and the constructor have type parameters, the class parameters are
     * prepended before those of the constructor's. This is an arbitrary rule since
     * no existing language spec mandates one way or the other. From the declaration
     * syntax, the class type parameter appears first, but the call syntax may show
     * up in opposite order such as {@code new <A>Foo<B>()}.
     */
    @Override
    public final TypeVariable<?>[] getTypeParameters() {
    ...
    private boolean mayNeedHiddenThis() {
      ...
      if (enclosingMethod != null) {
        ...
        // initializer,
        // and the class's constructor's first parameter happens to be the enclosing
        // class.
