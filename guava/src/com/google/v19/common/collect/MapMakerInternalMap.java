...
import static com.google.v19.common.collect.CollectPreconditions.checkRemove;
...
import com.google.v19.common.collect.GenericMapMaker.NullListener;
import com.google.v19.common.collect.MapMaker.RemovalCause;
import com.google.v19.common.collect.MapMaker.RemovalListener;
import com.google.v19.common.collect.MapMaker.RemovalNotification;
...
class MapMakerInternalMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V>, Serializable {
  ...
  enum EntryFactory {
    ...
    /**
     * Look-up table for factories. First dimension is the reference type. The
     * second dimension is the result of OR-ing the feature masks.
     */
    static final EntryFactory[][] factories = {
        { STRONG, STRONG_EXPIRABLE, STRONG_EVICTABLE, STRONG_EXPIRABLE_EVICTABLE }, {}, // no support for SOFT keys
  ...
  interface ValueReference<K, V> {
    ...
    /**
     * Waits for a value that may still be computing. Unlike get(), this method can
     ...
     *
     * @param newValue the new value reference which will replace this one; this is
     *                 only used during computation to immediately notify blocked
     *                 threads of the new value
     */
    void clear(@Nullable ValueReference<K, V> newValue);

    /**
     * Returns {@code true} if the value type is a computing reference (regardless
     * of whether or not computation has completed). This is necessary to distiguish
     * between partially-collected entries and computing entries, which need to be
   ...
   * key/value was partially collected, but not yet cleaned up
   */
  interface ReferenceEntry<K, V> {
    ...
    /*
     * Used by entries that are expirable. Expirable entries are maintained in a
    ...
    /*
     * Implemented by entries that are evictable. Evictable entries are maintained
     * in a doubly-linked list. New entries are added at the tail of the list at
     * write time and stale entries are expired from the head of the list.
     */
   ...
   * partially-collected, computing, or expired. Unlike
   * {@link Segment#getLiveValue} this method does not attempt to clean up stale
   * entries.
   */
  ...
  static class Segment<K, V> extends ReentrantLock {
     ...
     * the eviction list's ordering. It is drained as a batch operation when either
     ...
     * are added to the tail of the queue on access/write.
     */
    @GuardedBy("this")
    final Queue<ReferenceEntry<K, V>> evictionQueue;

    /**
     * A queue of elements currently in the map, ordered by expiration time (either
     * access or write time). Elements are added to the tail of the queue on
     * access/write.
     */
    @GuardedBy("this")
    ...
    Segment(MapMakerInternalMap<K, V> map, int initialCapacity, int maxSegmentSize) {
      ...
      evictionQueue = map.evictsBySize() ? new EvictionQueue<K, V>()
          : MapMakerInternalMap.<ReferenceEntry<K, V>>discardingQueue();

      expirationQueue = map.expires() ? new ExpirationQueue<K, V>()
          : MapMakerInternalMap.<ReferenceEntry<K, V>>discardingQueue();
    }
    ...
    /**
     * Performs eviction if the segment is full. This should only be called prior to
     * adding a new entry and increasing {@code count}.
     *
     ...
     * decrement count for all partially collected entries which are skipped. As
     * such callers which are modifying count must re-read it after calling
     * removeFromChain.
     *
    ...
    /**
     * Clears a value that has not yet been set, and thus does not require count to
     * be modified.
     */
     ...
     * the value is null and it is not computing.
     */
   ...
   * in the middle of the queue as part of copyEvictableEntry, and (2) the
  ...
  /**
   * Returns the internal entry for the specified key. The entry may be computing,
   * expired, or partially collected. Does not impact recency ordering.
   */
  ...
  abstract class HashIterator<E> implements Iterator<E> {
    ...
    /**
     * Finds the next entry in the current chain. Returns {@code true} if an entry
     * was found.
     */
    boolean nextInChain() {
      if (nextEntry != null) {
    ...
    /**
     * Finds the next entry in the current table. Returns {@code true} if an entry
     * was found.
     */
    boolean nextInTable() {
  ...
  Object writeReplace() {
    return new SerializationProxy<K, V>(keyStrength, valueStrength, keyEquivalence, valueEquivalence,
        expireAfterWriteNanos, expireAfterAccessNanos, maximumSize, concurrencyLevel, removalListener, this);
  }

  /**
   * The actual object that gets serialized. Unfortunately, readResolve() doesn't
   * get called when a circular dependency is present, so the proxy must be able
   * to behave as the map itself.
   */
  abstract static class AbstractSerializationProxy<K, V> extends ForwardingConcurrentMap<K, V> implements Serializable {
    ...
    AbstractSerializationProxy(Strength keyStrength, Strength valueStrength, Equivalence<Object> keyEquivalence,
        Equivalence<Object> valueEquivalence, long expireAfterWriteNanos, long expireAfterAccessNanos, int maximumSize,
        int concurrencyLevel, RemovalListener<? super K, ? super V> removalListener, ConcurrentMap<K, V> delegate) {
    ...
    MapMaker readMapMaker(ObjectInputStream in) throws IOException {
      ...
      MapMaker mapMaker = new MapMaker().initialCapacity(size).setKeyStrength(keyStrength)
          .setValueStrength(valueStrength).keyEquivalence(keyEquivalence).concurrencyLevel(concurrencyLevel);
  ...
  private static final class SerializationProxy<K, V> extends AbstractSerializationProxy<K, V> {
    ...
    SerializationProxy(Strength keyStrength, Strength valueStrength, Equivalence<Object> keyEquivalence,
        Equivalence<Object> valueEquivalence, long expireAfterWriteNanos, long expireAfterAccessNanos, int maximumSize,
        int concurrencyLevel, RemovalListener<? super K, ? super V> removalListener, ConcurrentMap<K, V> delegate) {
      super(keyStrength, valueStrength, keyEquivalence, valueEquivalence, expireAfterWriteNanos, expireAfterAccessNanos,
