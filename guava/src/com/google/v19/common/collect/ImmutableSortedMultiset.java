...
/**
 * A {@link SortedMultiset} whose contents will never change, with many other
 ...
 * {@code a.equals(b)}. If this advice is not followed, the resulting collection
 * will not correctly obey its specification.
 *
 * <p>
...
public abstract class ImmutableSortedMultiset<E> extends ImmutableSortedMultisetFauxverideShim<E>
  ...
  private static final ImmutableSortedMultiset<Comparable> NATURAL_EMPTY_MULTISET = new RegularImmutableSortedMultiset<Comparable>(
      NATURAL_ORDER);
  ...
  /**
   * Returns an immutable sorted multiset containing the given elements sorted by
  ...
  /**
   * Returns an immutable sorted multiset containing the given elements sorted by
   * their natural ordering. To create a copy of a {@code SortedMultiset} that
   * preserves the comparator, call {@link #copyOfSorted} instead. This method
   ...
   * {@code ImmutableSortedMultiset<multiset<String>>} containing one element (the
  ...
  /**
   * Returns an immutable sorted multiset containing the elements of a sorted
   * multiset, sorted by the same {@code Comparator}. That behavior differs from
   ...
   *
   * <p>
   * This method is safe to use even when {@code sortedMultiset} is a synchronized
   * or concurrent collection that is currently being modified by another thread.
   *
   * @throws NullPointerException if {@code sortedMultiset} or any of its elements
   *                              is null
   */
  ...
  private static <E> ImmutableSortedMultiset<E> copyOfSortedEntries(Comparator<? super E> comparator,
    ...
    return new RegularImmutableSortedMultiset<E>(new RegularImmutableSortedSet<E>(elementsBuilder.build(), comparator),
  ...
  public ImmutableSortedMultiset<E> subMultiset(E lowerBound, BoundType lowerBoundType, E upperBound,
    ...
    checkArgument(comparator().compare(lowerBound, upperBound) <= 0, "Expected lowerBound <= upperBound but %s > %s",
   ...
   * generated, such as creating a {@code
   * SortedMultiset<Integer>} with a {@code Comparator<Number>}, use the
   ...
   *
   * <p>
   * Note: the type parameter {@code E} extends {@code Comparable<?>} rather than
   * {@code
   ...
   *   &#64;code
   *
   *   public static final ImmutableSortedMultiset<Bean> BEANS = new ImmutableSortedMultiset.Builder<Bean>()
