...
/**
 * An expanded {@code Iterable} API, providing functionality similar to Java 8's
 * powerful <a href=
 ...
 *
 * <p>
 * Starting with Java 8, the core Java class libraries provide a new "Streams"
 * library (in {@code
 * java.util.stream}), which is similar to {@code FluentIterable} but generally
 * more powerful. Key differences include:</b>
 *
 * <ul>
 ...
 * {@code
 *     distinct}, {@code reduce}, {@code sorted}, the very powerful
 ...
 * <li>Streams are standard Java, not requiring a third-party dependency (but do
 * render your code incompatible with Java 7 and earlier).
 * </ul>
 *
 * <h3>Example</h3>
 *
 ...
 *   &#64;code
 *
 *   List<String> results = FluentIterable.from(database.getClientList()).filter(activeInLastMonthPredicate)
 ...
 *   &#64;code
 *
 *   List<String> results = database.getClientList().stream().filter(activeInLastMonthPredicate)
 *       .map(Functions.toStringFunction()).limit(10).collect(Collectors.toList());
 * }
 * </pre>
 *
...
public abstract class FluentIterable<E> implements Iterable<E> {
   ...
   *
   * <p>
   * <b>{@code Stream} equivalent:</b> {@code iterable.stream()} if
   * {@code iterable} is a {@link Collection};
   * {@code StreamSupport.stream(iterable.spliterator(), false)} otherwise.
   */
  @CheckReturnValue
   ...
   *
   * <p>
   * <b>{@code Stream} equivalent:</b> {@code Stream.of(elements)} or {@code
   ...
   * efficiently) {@code collect(Collectors.toList()).toString()}.
   */
  @Override
  @CheckReturnValue
   ...
   *
   * <p>
   * <b>{@code Stream} equivalent:</b> {@code stream.count()}.
   */
  @CheckReturnValue
   ...
   * element {@code
   * element}, use {@code Stream.generate(() -> element)}. Otherwise, if the
   * source iterable has a {@code stream} method (for example, if it is a
   * {@link Collection}), use
   ...
   * <p>
   * <b>{@code Stream} equivalent:</b>
   * {@code Stream.concat(thisStream, otherStream)}.
   *
   * @since 18.0
   */
  @Beta
   ...
   *
   * <p>
   * <b>{@code Stream} equivalent:</b> {@code stream.filter(predicate)} (same).
   */
  @CheckReturnValue
   ...
   *   Stream<T> result = (Stream) stream.filter(type::isInstance);}
   * </pre>
   *
   ...
   *
   *   &#64;SuppressWarnings("unchecked") // safe by runtime check
   *   Stream<MyType> result = (Stream) stream.filter(e -> e instanceof MyType);}
   * </pre>
   */
   ...
   *
   * <p>
   * <b>{@code Stream} equivalent:</b> {@code stream.anyMatch(predicate)} (same).
   */
  @CheckReturnValue
   ...
   *
   * <p>
   * <b>{@code Stream} equivalent:</b> {@code stream.allMatch(predicate)} (same).
   */
  @CheckReturnValue
   ...
   *
   * <p>
   * <b>{@code Stream} equivalent:</b> {@code stream.map(function)}.
   */
  @CheckReturnValue
   ...
   *
   * <p>
   * <b>{@code Stream} equivalent:</b> {@code stream.flatMap(function)} (using a
   ...
   * {@code
   * stream.findAny()}; if it must specifically be the <i>first</i> element,
   * {@code
   * stream.findFirst()}.
   *
   ...
   *
   * <p>
   * <b>{@code Stream} equivalent:</b> {@code stream.skip(numberToSkip)} (same).
   */
  @CheckReturnValue
   ...
   *
   * <p>
   * <b>{@code Stream} equivalent:</b> {@code stream.limit(maxSize)} (same).
   *
   ...
   * <p>
   * <b>{@code Stream} equivalent:</b>
   * {@code ImmutableList.copyOf(stream.iterator())}.
   *
   ...
   * <p>
   * <b>{@code Stream} equivalent:</b>
   * {@code ImmutableSet.copyOf(stream.iterator())}.
   *
   ...
   * <p>
   * <b>{@code Stream} equivalent:</b>
   * {@code ImmutableMultiset.copyOf(stream.iterator())}.
   *
   * @since 19.0
   */
  @CheckReturnValue
   ...
   * ImmutableMap.copyOf(stream.collect(Collectors.toMap(k -> k, valueFunction)))}
   * (but note that this may not preserve the order of entries).
   *
   ...
   *                              <p>
   *                              <b>{@code Stream} equivalent:</b>
   *                              {@code stream.collect(Collectors.groupingBy(keyFunction))}
   *                              behaves similarly, but returns a mutable
   *                              {@code Map<K, List<E>>} instead, and may not
   ...
   * <p>
   * <b>{@code Stream} equivalent:</b> {@code
   * ImmutableMap.copyOf(stream.collect(Collectors.toMap(keyFunction, v -> v)))}
   * (but note that this may not preserve the order of entries).
   *
   * @param keyFunction the function used to produce the key for each value
   ...
   *                                  iterable
   * @throws NullPointerException     if any elements of this fluent iterable is
   ...
   * {@code MyType.class}, use {@code stream.toArray(MyType[]::new)}. Otherwise
   * use {@code stream.toArray(
   ...
   * if you are not using any optional {@code Joiner} features,
