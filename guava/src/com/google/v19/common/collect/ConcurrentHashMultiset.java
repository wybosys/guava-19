...
import static com.google.v19.common.collect.CollectPreconditions.checkNonnegative;
...
import com.google.v19.common.collect.Serialization.FieldSetter;
import com.google.v19.common.math.IntMath;
...
public final class ConcurrentHashMultiset<E> extends AbstractMultiset<E> implements Serializable {
   ...
   * terms of AtomicInteger's atomic operations, with some help from
   * ConcurrentMap's atomic operations on creation and removal (including
   * automatic removal of zeroes). If the modification of an AtomicInteger results
   * in zero, we compareAndSet the value to zero; if that succeeds, we remove the
   * entry from the Map. If another operation sees a zero in the map, it knows
   * that the entry is about to be removed, so this operation may remove it (often
   * by replacing it with a new AtomicInteger).
   */
  ...
  /**
   * Creates a new, empty {@code ConcurrentHashMultiset} using {@code mapMaker} to
   * construct the internal backing map.
   *
   * <p>
   * If this {@link MapMaker} is configured to use entry eviction of any kind,
   * this eviction applies to all occurrences of a given element as a single unit.
   * However, most updates to the multiset do not count as map updates at all,
   * since we're usually just mutating the value stored in the map, so
   * {@link MapMaker#expireAfterAccess} makes sense (evict the entry that was
   * queried or updated longest ago), but {@link MapMaker#expireAfterWrite}
   * doesn't, because the eviction time is measured from when we saw the first
   * occurrence of the object.
   *
   * <p>
   * The returned multiset is serializable but any serialization caveats given in
   * {@code MapMaker} apply.
   *
   * <p>
   * Finally, soft/weak values can be used but are not very useful: the values are
   * created internally and not exposed externally, so no one else will have a
   * strong reference to the values. Weak keys on the other hand can be useful in
   * some scenarios.
   *
   * @since 15.0 (source compatible (accepting the since removed
   *        {@code GenericMapMaker} class) since 7.0)
   */
  @Beta
  ...
  /*
   * We'd love to use 'new ArrayList(this)' or 'list.addAll(this)', but either of
   * these would recurse back to us again!
   */
   ...
   *                                  the resulting amount would exceed
   ...
   * CollectionRemoveTester.testRemove_nullAllowed, but it's not clear that it's a
   * good policy, especially because, in order for the test to pass, the parameter
   * must be misleadingly annotated as @Nullable. I suspect that we'll want to
   * remove @Nullable, add an eager checkNotNull, and loosen up
   ...
   *
   * <p>
   * This method, in contrast to {@link #remove(Object, int)}, has no effect when
   * the element count is smaller than {@code occurrences}.
   *
   ...
   * only if the count is currently {@code expectedOldCount}. If {@code element}
   * does not appear in the multiset exactly {@code expectedOldCount} times, no
   ...
   *         {@code expectedOldCount == newCount}, the method will return
   *         {@code true} if the condition was met.
   * @throws IllegalArgumentException if {@code expectedOldCount} or
   *                                  {@code newCount} is negative
   */
  @Override
  public boolean setCount(E element, int expectedOldCount, int newCount) {
    ...
    if (existingCounter == null) {
      if (expectedOldCount != 0) {
        ...
        // if our write lost the race, it must have lost to a nonzero value, so we can
        // stop
