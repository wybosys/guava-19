...
public final class MinMaxPriorityQueue<E> extends AbstractQueue<E> {
   ...
   *            builder (for example, a {@code Builder<Number>} can produce a
  ...
  public static final class Builder<B> {
    ...
    /**
     * Configures this builder to build {@code MinMaxPriorityQueue} instances that
     * are limited to {@code maximumSize} elements. Each time a queue grows beyond
     * this bound, it immediately removes its greatest element (according to its
   ...
   * element (according to its comparator), which may be {@code
   * element} itself.
   *
   ...
   *
   * <p>
   * Occasionally, in order to maintain the heap invariant, it must swap a later
   * element of the list with one before {@code index}. Under these circumstances
   * it returns a pair of elements as a {@link MoveDesc}. The first one is the
   * element that was previously at the end of the heap and is now at some
   * position before {@code index}. The second element is the one that was swapped
   * down to replace the element at {@code index}. This fact is used by
   * iterator.remove so as to visit elements during a traversal once and only
   * once.
   */
  @VisibleForTesting
