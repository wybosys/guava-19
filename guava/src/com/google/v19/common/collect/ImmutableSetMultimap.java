...
/**
 * A {@link SetMultimap} whose contents will never change, with many other
...
public class ImmutableSetMultimap<K, V> extends ImmutableMultimap<K, V> implements SetMultimap<K, V> {
  ...
  private static <K, V> ImmutableSetMultimap<K, V> copyOf(Multimap<? extends K, ? extends V> multimap,
    ...
    ImmutableMap.Builder<K, ImmutableSet<V>> builder = new ImmutableMap.Builder<K, ImmutableSet<V>>(
        multimap.asMap().size());
    int size = 0;
   ...
   * multimap iterates over keys in the order they were first encountered in the
   * input, and the values for each key are iterated in the order they were
   * encountered. If two values for the same key are {@linkplain Object#equals
   * equal}, the first value encountered is used.
   *
