 ...
 * it can build. For the most part, you should probably just ignore the
 ...
 * @since 7.0
 * @deprecated This class existed only to support the generic paramterization
 *             necessary for the caching functionality in {@code MapMaker}. That
 *             functionality has been moved to
 *             {@link com.google.v19.common.cache.CacheBuilder}, which is a
 *             properly generified class and thus needs no "Generic" equivalent;
 *             simple use {@code CacheBuilder} naturally. For general migration
...
abstract class GenericMapMaker<K0, V0> {
  ...
  /*
   * Note that MapMaker's removalListener() is not here, because once you're
   * interacting with a GenericMapMaker you've already called that, and shouldn't
   * be calling it again.
   */
