...
import static com.google.v19.common.collect.ObjectArrays.checkElementNotNull;
...
/**
 * A {@link Set} whose contents will never change, with many other important
...
public abstract class ImmutableSet<E> extends ImmutableCollection<E> implements Set<E> {
  /**
   * Returns the empty immutable set. Preferred over {@link Collections#emptySet}
   * for code consistency, and because the return type conveys the immutability
   * guarantee.
   */
   ...
   * {@link Collections#singleton} for code consistency, {@code null} rejection,
   * and because the return type conveys the immutability guarantee.
   */
  ...
  /**
   * Returns an immutable set containing the given elements, minus duplicates, in
   * the order each was first specified. That is, if multiple elements are
  ...
  /**
   * Returns an immutable set containing each of {@code elements}, minus
   * duplicates, in the order each appears first in the source collection.
   *
   * <p>
   * <b>Performance note:</b> This method will sometimes recognize that the actual
   * copy operation is unnecessary; for example,
   * {@code copyOf(copyOf(anArrayList))} will copy the data only once. This
   * reduces the expense of habitually making defensive copies at API boundaries.
   * However, the the precise conditions for skipping the copy operation are
   * undefined.
   *
  ...
  /**
   * Returns an immutable set containing each of {@code elements}, minus
   * duplicates, in the order each appears first in the source iterable. This
   * method iterates over {@code elements} only once.
   *
   * <p>
   * <b>Performance note:</b> This method will sometimes recognize that the actual
   * copy operation is unnecessary; for example,
   * {@code copyOf(copyOf(anArrayList))} should copy the data only once. This
   * reduces the expense of habitually making defensive copies at API boundaries.
   * However, the precise conditions for skipping the copy operation are
   * undefined.
   *
   * @throws NullPointerException if any of {@code elements} is null
   */
  ...
  /**
   * Returns an immutable set containing each of {@code elements}, minus
   * duplicates, in the order each appears first in the source iterator.
   *
   * @throws NullPointerException if any of {@code elements} is null
   */
  ...
  /**
   * Returns an immutable set containing each of {@code elements}, minus
   * duplicates, in the order each appears first in the source array.
   *
   * @throws NullPointerException if any of {@code elements} is null
   * @since 3.0
   */
   ...
   *   &#64;code
   *
   *   static final ImmutableSet<Color> GOOGLE_COLORS = ImmutableSet.<Color>builder().addAll(WEBSAFE_COLORS)
   *       .add(new Color(0, 191, 255)).build();
   * }
   * </pre>
   *
   * <p>
   * Building does not change the state of the builder, so it is still possible to
   * add more elements and to build again.
   *
   * @since 2.0
   */
