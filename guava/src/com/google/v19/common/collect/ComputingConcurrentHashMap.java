...
class ComputingConcurrentHashMap<K, V> extends MapMakerInternalMap<K, V> {
  ...
  /**
   * Creates a new, empty map with the specified strategy, initial capacity, load
  ...
  static final class ComputingSegment<K, V> extends Segment<K, V> {
    ...
    V compute(K key, int hash, ReferenceEntry<K, V> e, ComputingValueReference<K, V> computingValueReference)
      ...
      long end = 0;
      try {
        // Synchronizes on the entry to allow failing fast when a recursive computation
        // is
        // detected. This is not fool-proof since the entry may be copied when the
        // segment
        // is written to.
        synchronized (e) {
  ...
  @Override
  Object writeReplace() {
    return new ComputingSerializationProxy<K, V>(keyStrength, valueStrength, keyEquivalence, valueEquivalence,
  ...
  static final class ComputingSerializationProxy<K, V> extends AbstractSerializationProxy<K, V> {
    ...
    ComputingSerializationProxy(Strength keyStrength, Strength valueStrength, Equivalence<Object> keyEquivalence,
