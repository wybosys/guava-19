...
public final class Queues {
  ...
  // LinkedList: see {@link com.google.v19.common.collect.Lists}
   ...
   * the requested {@code numElements} elements are not available, it will wait
   * for them up to the specified timeout.
   *
   ...
   * interruption status will be set (no {@code InterruptedException} is thrown).
   *
   * @param q           the blocking queue to be drained
   * @param buffer      where to add the transferred elements
   * @param numElements the number of elements to be waited for
  ...
  /**
   * Returns a synchronized (thread-safe) queue backed by the specified queue. In
   * order to guarantee serial access, it is critical that <b>all</b> access to
   * the backing queue is accomplished through the returned queue.
   *
   * <p>
   * It is imperative that the user manually synchronize on the returned queue
   * when accessing the queue's iterator:
   * 
   * <pre>
   *    {@code
   *
  ...
  /**
   * Returns a synchronized (thread-safe) deque backed by the specified deque. In
   * order to guarantee serial access, it is critical that <b>all</b> access to
   * the backing deque is accomplished through the returned deque.
   *
   * <p>
   * It is imperative that the user manually synchronize on the returned deque
   * when accessing any of the deque's iterators:
   * 
   * <pre>
   *    {@code
   *
