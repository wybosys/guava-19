...
public final class Throwables {
   ...
   * in a {@code RuntimeException} and then propagates.
   * <p>
   ...
   * will be {@code
   * throwable} followed by its cause hierarchy. Note that this is a snapshot of
   * the cause chain and will not reflect any subsequent changes to the cause
   * chain.
   *
   * <p>
   ...
   * iteration over the full trace but faster iteration over parts of the trace.
   * Here, "slower" and "faster" are defined in comparison to the normal way to
   * access the stack trace, {@link Throwable#getStackTrace()
   * throwable.getStackTrace()}. Note, however, that this method's special
   * implementation is not available for all platforms and configurations. If that
   * implementation is unavailable, this method falls back to
   * {@code getStackTrace}. Callers that require the special implementation can
   * check its availability with {@link #lazyStackTraceIsLazy()}.
   *
   * <p>
   * The expected (but not guaranteed) performance of the special implementation
   * differs from {@code getStackTrace} in one main way: The
   ...
   * (probably slightly more than {@code 1/stackSize}).
   * </ul>
   *
   * <p>
  ...
  /**
   * The "getStackTraceElementMethod" method, only available on some JDKs so we
   * use reflection to find it when available. When this is null, use the slow
   * way.
   */
  @Nullable
   ...
   * whitelisted for AppEngine, and not present in non-Sun JDKs.
   */
  @Nullable
  private static Object getJLA() {
    try {
       ...
       * these classes and would result in compilation failure if we directly refer to
       * these classes.
       */
      ...
      /*
       * This is not one of AppEngine's whitelisted classes, so even in Sun JDKs, this
   ...
   * StackTraceElement, or null if that method cannot be found (it is only to be
   * found in fairly recent JDKs).
   */
  @Nullable
  ...
  /**
   * Returns the Method that can be used to return the size of a stack, or null if
