...
package com.google.v19.common.hash;
...
public final class Hashing {
  ...
  /**
   * Returns a hash function implementing the SHA-384 algorithm (384 hash bits) by
   * delegating to the SHA-384 {@link MessageDigest}.
   *
   * @since 19.0
   */
  ...
  /**
   * Returns a hash function implementing the CRC32C checksum algorithm (32 hash
   * bits) as described by RFC 3720, Section 12.1.
   *
   * @since 18.0
   */
   ...
   * dividing traffic among tasks, you can decrease the number of tasks from 15
   * and 10, killing off the final 5 tasks, and {@code
   * consistentHash} will handle it. If, however, you are dividing traffic among
   * servers {@code
   * alpha}, {@code bravo}, and {@code charlie} and you occasionally need to take
   * each of the servers offline, {@code consistentHash} will be a poor fit: It
   * provides no way for you to specify which of the three buckets is
   * disappearing. Thus, if your buckets change from {@code
   * [alpha, bravo, charlie]} to {@code [bravo, charlie]}, it will assign all the
   * old {@code alpha} traffic to {@code bravo} and all the old {@code bravo}
   * traffic to {@code charlie}, rather than letting {@code bravo} keep its
   * traffic.
   * </ul>
   *
   *
   ...
   * hash codes of the underlying hash functions together. This can be useful if
   * you need to generate hash codes of a specific length.
   *
   * <p>
  ...
  private static final class ConcatenatedHashFunction extends AbstractCompositeHashFunction {
    ...
    private ConcatenatedHashFunction(HashFunction... functions) {
      ...
      for (HashFunction function : functions) {
        ...
        checkArgument(function.bits() % 8 == 0, "the number of bits (%s) in hashFunction (%s) must be divisible by 8",
