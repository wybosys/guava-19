...
import com.google.v19.common.hash.Hashing;
import com.google.v19.common.io.ByteStreams;
...
public final class InetAddresses {
   ...
   * @throws IllegalArgumentException if a valid {@link Inet4Address} can not be
   *                                  created
   */
   ...
   * exception "if IP address is of illegal length." We replace it with an
   ...
   * follows <a href="http://tools.ietf.org/html/rfc5952">RFC 5952</a> section 4.
   * The main difference is that this method uses "::" for zero compression, while
   * Java's version uses the uncompressed form.
   *
   * <p>
   * This method uses hexadecimal for all IPv6 addresses, including IPv4-mapped
   * IPv6 addresses such as "::c000:201". The output does not include a Scope ID.
   *
   ...
   * leftmost run wins. If a qualifying run is found, its hextets are replaced by
  ...
  private static String hextetsToIPv6String(int[] hextets) {
    /*
     * While scanning the array, handle these state transitions: start->num => "num"
     * start->gap => "::" num->num => ":num" num->gap => "::" gap->num => "num"
     * gap->gap => ""
     */
  ...
  /**
   * Returns the string representation of an {@link InetAddress} suitable for
   * inclusion in a URI.
   *
   * <p>
   * For IPv4 addresses, this is identical to
   * {@link InetAddress#getHostAddress()}, but for IPv6 addresses it compresses
   * zeroes and surrounds the text with square brackets; for example
   * {@code "[2001:db8::1]"}.
   *
   * <p>
   * Per section 3.2.2 of
   * <a target="_parent" href="http://tools.ietf.org/html/rfc3986#section-3.2.2"
   * >http://tools.ietf.org/html/rfc3986</a>, a URI containing an IPv6 string
   * literal is of the form {@code "http://[2001:db8::1]:8888/index.html"}.
   *
   * <p>
   * Use of either {@link InetAddresses#toAddrString},
   * {@link InetAddress#getHostAddress()}, or this method is recommended over
   * {@link InetAddress#toString()} when an IP address string literal is desired.
   * This is because {@link InetAddress#toString()} prints the hostname and the IP
  ...
  /**
   * Returns an InetAddress representing the literal IPv4 or IPv6 host portion of
   * a URL, encoded in the format specified by RFC 3986 section 3.2.2.
   *
   * <p>
   * This function is similar to {@link InetAddresses#forString(String)}, however,
   * it requires that IPv6 addresses are surrounded by square brackets.
   *
   * <p>
   ...
   *                                  address, or IPv6 address surrounded by
   *                                  square brackets
   */
   ...
   * zero, with the remaining 32 bits interpreted as an IPv4 address. These are
   * conventionally represented in string literals as {@code "::192.168.0.1"},
   * though {@code "::c0a8:1"} is also considered an IPv4 compatible address (and
   * equivalent to {@code "::192.168.0.1"}).
   *
   * <p>
   * For more on IPv4 compatible addresses see section 2.5.5.1 of
   * <a target="_parent" href="http://tools.ietf.org/html/rfc4291#section-2.5.5.1"
   ...
   * considered to be).
   *
   * @param ip {@link Inet6Address} to be examined for embedded IPv4 compatible
   *           address format
   ...
   * @throws IllegalArgumentException if the argument is not a valid IPv4
   *                                  compatible address
   */
   ...
   *
   * <p>
   * 6to4 addresses begin with the {@code "2002::/16"} prefix. The next 32 bits
   * are the IPv4 address of the host to which IPv6-in-IPv4 tunneled packets
   * should be routed.
   *
   * <p>
   ...
   *
   * @param ip {@link Inet6Address} to be examined for embedded IPv4 in 6to4
   *           address
   * @return {@link Inet4Address} of embedded IPv4 in 6to4 address
   * @throws IllegalArgumentException if the argument is not a valid IPv6 6to4
   *                                  address
   */
   ...
   *
   * <p>
   * All of the fields in this class are encoded in various portions of the IPv6
   * address as part of the protocol. More protocols details can be found at:
  ...
  public static final class TeredoInfo {
     ...
     *                                  {@code flags} arguments are out of range of
     *                                  an unsigned short
     */
    // TODO: why is this public?
    public TeredoInfo(@Nullable Inet4Address server, @Nullable Inet4Address client, int port, int flags) {
      Preconditions.checkArgument((port >= 0) && (port <= 0xffff), "port '%s' is out of range (0 <= port <= 0xffff)",
          port);
   ...
   * @throws IllegalArgumentException if the argument is not a valid IPv6 Teredo
   *                                  address
   */
   ...
   *
   * <p>
   * From RFC 5214: "ISATAP interface identifiers are constructed in Modified
   * EUI-64 format [...] by concatenating the 24-bit IANA OUI (00-00-5E), the
   * 8-bit hexadecimal value 0xFE, and a 32-bit IPv4 address in network byte order
   * [...]"
   *
   * <p>
   ...
   *
   * @param ip {@link Inet6Address} to be examined for embedded IPv4 in ISATAP
   *           address
   * @return {@link Inet4Address} of embedded IPv4 in an ISATAP address
   * @throws IllegalArgumentException if the argument is not a valid IPv6 ISATAP
   *                                  address
   */
   ...
   * trivial spoofability. With other transition addresses spoofing involves (at
   * least) infection of one's BGP routing table.
   *
   * @param ip {@link Inet6Address} to be examined for embedded IPv4 client
   *           address
   ...
   * InetAddress is an IPv6 address of one of the specified address types that
   ...
   *
   * <p>
   * An "IPv4 mapped" address is anything in the range ::ffff:0:0/96 (sometimes
   * written as ::ffff:0.0.0.0/96), with the last 32 bits interpreted as an IPv4
   * address.
   *
   * <p>
   * For more on IPv4 mapped addresses see section 2.5.5.2 of
   * <a target="_parent" href="http://tools.ietf.org/html/rfc4291#section-2.5.5.2"
   * >http://tools.ietf.org/html/rfc4291</a>
   *
   * <p>
   * Note: This method takes a {@code String} argument because {@link InetAddress}
   * automatically collapses mapped addresses to IPv4. (It is actually possible to
   * avoid this using one of the obscure {@link Inet6Address} methods, but it
   * would be unwise to depend on such a poorly-documented feature.)
   *
   * @param ipString {@code String} to be examined for embedded IPv4-mapped IPv6
   *                 address format
   ...
   * into tables, accounting, et cetera, it may be necessary to <b>coerce</b> IPv6
   * addresses into IPv4 addresses. This function does so by hashing the upper 64
   * bits into {@code 224.0.0.0/3} (64 bits into 29 bits).
   *
   * <p>
  ...
  /**
   * Returns an integer representing an IPv4 address regardless of whether the
   * supplied argument is an IPv4 address or not.
   *
   * <p>
   ...
   * addresses and can therefore be converted safely to integers (for whatever
   * purpose) this function can be used to handle IPv6 addresses as well until the
   * application is suitably fixed.
   *
   * <p>
   * NOTE: an IPv6 address coerced to an IPv4 address can only be used for such
   * purposes as rudimentary identification or indexing into a collection of real
   * {@link InetAddress}es. They cannot be used as real addresses for the purposes
  ...
  /**
   * Returns an address from a <b>little-endian ordered</b> byte array (the
   * opposite of what {@link InetAddress#getByAddress} expects).
   *
   * <p>
  ...
  /**
   * Returns a new InetAddress that is one less than the passed in address. This
   * method works for both IPv4 and IPv6 addresses.
   *
   * @param address the InetAddress to decrement
   * @return a new InetAddress that is one less than the passed in address
   * @throws IllegalArgumentException if InetAddress is at the beginning of its
   *                                  range
   * @since 18.0
   */
  ...
  /**
   * Returns a new InetAddress that is one more than the passed in address. This
   * method works for both IPv4 and IPv6 addresses.
   *
