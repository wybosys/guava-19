...
import com.google.v19.common.base.Strings;
...
public final class HostAndPort implements Serializable {
   ...
   *                               {@link #withDefaultPort(int)} to prevent this
   *                               from occurring.
   */
  public int getPort() {
    checkState(hasPort());
   ...
   *
   * Note that the host-only formats will leave the port field undefined. You can
   * use {@link #withDefaultPort(int)} to patch in a default value.
   *
   ...
   * parsing fails.
   *
   * @param hostPortString the full bracketed host-port specification. Post might
   *                       not be specified.
   * @return an array with 2 strings: host and port, in that order.
   * @throws IllegalArgumentException if parsing the bracketed host-port string
   *                                  fails.
   */
  private static String[] getHostAndPortFromBracketedHost(String hostPortString) {
    ...
    if (closeBracketIndex + 1 == hostPortString.length()) {
      ...
      checkArgument(hostPortString.charAt(closeBracketIndex + 1) == ':', "Only a colon may follow a close bracket: %s",
          hostPortString);
   ...
   * "[2001:db8::1]". Chain this call after {@link #fromString(String)} to
   * increase the strictness of the parser, and disallow IPv6 literals that don't
   * contain these brackets.
   *
   * <p>
   * Note that this parser identifies IPv6 literals solely based on the presence
   * of a colon. To perform actual validation of IP addresses, see the
