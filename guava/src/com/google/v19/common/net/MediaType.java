...
package com.google.v19.common.net;

import static com.google.v19.common.base.CharMatcher.ASCII;
import static com.google.v19.common.base.CharMatcher.JAVA_ISO_CONTROL;
import static com.google.v19.common.base.Charsets.UTF_8;
...
import com.google.v19.common.base.CharMatcher;
import com.google.v19.common.base.Function;
import com.google.v19.common.base.Joiner;
import com.google.v19.common.base.Joiner.MapJoiner;
import com.google.v19.common.base.MoreObjects;
import com.google.v19.common.base.Objects;
import com.google.v19.common.base.Optional;
import com.google.v19.common.collect.ImmutableListMultimap;
import com.google.v19.common.collect.ImmutableMultiset;
import com.google.v19.common.collect.ImmutableSet;
import com.google.v19.common.collect.Iterables;
import com.google.v19.common.collect.Maps;
import com.google.v19.common.collect.Multimap;
 ...
 * Type</a> (also known as a MIME Type or Content Type). This class also
 * supports the concept of media ranges <a href=
 * "http://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html#sec14.1">defined by
 * HTTP/1.1</a>. As such, the {@code *} character is treated as a wildcard and
 * is used to represent any acceptable type or subtype value. A media type may
 * not have wildcard type with a declared subtype. The {@code *} character has
 * no special meaning as part of a parameter. All values for type, subtype,
 ...
 * {@code charset} parameter is normalized to lowercase, but all others are left
 * as-is.
 *
 * <p>
 * Note that this specifically does <strong>not</strong> represent the value of
 * the MIME {@code Content-Type} header and as such has no support for
 * header-specific considerations such as line folding and comments.
 *
 * <p>
 * For media types that take a charset the predefined constants default to UTF-8
 * and have a "_UTF_8" suffix. To get a version without a character set, use
...
public final class MediaType {
  ...
  private static final CharMatcher TOKEN_MATCHER = ASCII.and(JAVA_ISO_CONTROL.negate()).and(CharMatcher.isNot(' '))
      .and(CharMatcher.noneOf("()<>@,;:\\\"/[]?="));
  private static final CharMatcher QUOTED_TEXT_MATCHER = ASCII.and(CharMatcher.noneOf("\"\\\r"));
  /*
   * This matches the same characters as linear-white-space from RFC 822, but we
   * make no effort to enforce any particular rules with regards to line folding
   * as stated in the class docs.
   */
  ...
  /*
   * The following constants are grouped by their type and ordered alphabetically
   * by the constant name within that type. The constant name should be a sensible
   * identifier that is closest to the "common name" of the media. This is often,
   * but not necessarily the same as the subtype.
   *
   * Be sure to declare all constants with the type and subtype in all lowercase.
   * For types that take a charset (e.g. all text/* types), default to UTF-8 and
   * suffix the constant name with "_UTF_8".
   */
   ...
   * {@link #JAVASCRIPT_UTF_8 application/javascript} to be the correct media type
   * for JavaScript, but this may be necessary in certain situations for
   * compatibility.
   */
  public static final MediaType TEXT_JAVASCRIPT_UTF_8 = createConstantUtf8(TEXT_TYPE, "javascript");
  /**
   * <a href=
   * "http://www.iana.org/assignments/media-types/text/tab-separated-values"> Tab
   * separated values</a>.
   *
   * @since 15.0
   */
  ...
  /**
   * As described in <a href="http://www.ietf.org/rfc/rfc3023.txt">RFC 3023</a>,
   * this constant ({@code text/xml}) is used for XML documents that are "readable
   * by casual users." {@link #APPLICATION_XML_UTF_8} is provided for documents
   * that are intended for applications.
   */
   ...
   * <a href="http://en.wikipedia.org/wiki/Camera_Image_File_Format">Canon Image
   * File Format</a> ({@code crw} files), a widely-used "raw image" format for
   * cameras. It is found in {@code /etc/mime.types}, e.g. in <href=
  ...
  /**
   * The media type for the Photoshop File Format ({@code psd} files) as defined
   * by <a href=
   ...
   * "http://en.wikipedia.org/wiki/Adobe_Photoshop#File_format">Wikipedia</a>;
   * this is the regular output/input of Photoshop (which can also export to
   * various image formats; note that files with extension "PSB" are in a distinct
   * but related format).
   * <p>
  ...
  /**
   * As described in <a href="http://www.ietf.org/rfc/rfc3023.txt">RFC 3023</a>,
   * this constant ({@code application/xml}) is used for XML documents that are
   * "unreadable by casual users." {@link #XML_UTF_8} is provided for documents
   * that may be read by users.
   */
  ...
  /**
   * Media type for
   * <a href="https://www.dartlang.org/articles/embedding-in-html/">dart
   * files</a>.
   *
   * @since 19.0
   */
  ...
  /**
   * Media type for
   * <a href="http://en.wikipedia.org/wiki/Embedded_OpenType">Embedded
   * OpenType</a> fonts. This is <a href=
  ...
  /**
   * As described in the <a href="http://idpf.org/epub">International Digital
   * Publishing Forum</a> EPUB is the distribution and interchange format standard
   * for digital publications and documents. This media type is defined in the
   * <a href="http://www.idpf.org/epub/30/spec/epub30-ocf.html">EPUB Open
   * Container Format</a> specification.
   *
   * @since 15.0
   */
  ...
  /**
   * As described in <a href="https://www.rsa.com/rsalabs/node.asp?id=2138">PKCS
   * #12: Personal Information Exchange Syntax Standard</a>, PKCS #12 defines an
  ...
  /**
   * This is a non-standard media type, but is commonly used in serving hosted
   * binary files as it is <a href=
   * "http://code.google.com/p/browsersec/wiki/Part2#Survey_of_content_sniffing_behaviors">
   * known not to trigger content sniffing in current browsers</a>. It <i>should
   * not</i> be used in other situations as it is not specified by any RFC and
   * does not appear in the
   * <a href= "http://www.iana.org/assignments/media-types">/IANA MIME Media
   * Types</a> list. Consider {@link #OCTET_STREAM} for binary data that is not
  ...
  /**
   * <a href="http://www.rfc-editor.org/rfc/rfc4329.txt">RFC 4329</a> declares
   * this to be the correct media type for JavaScript, but
   * {@link #TEXT_JAVASCRIPT_UTF_8 text/javascript} may be necessary in certain
  ...
  /**
   * Media type for the <a href="http://www.w3.org/TR/appmanifest/">Manifest for a
   * web application</a>.
   *
   * @since 19.0
   */
  ...
  /**
   * Media type for <a href="http://goo.gl/1pGBFm">Apple over-the-air mobile
  ...
  /**
   * <a href=
   * "http://tools.ietf.org/html/draft-rfernando-protocol-buffers-00">Protocol
   * buffers</a>
   *
   * @since 15.0
   */
   ...
   * <a href=
   * "http://www.iana.org/assignments/media-types/application/font-sfnt">registered</a>
   * with the IANA.
   *
   * @since 17.0
   */
   ...
   * Format</a> (WOFF) <a href="http://www.w3.org/TR/WOFF/">defined</a> by the
   * W3C. This is <a href=
   * "http://www.iana.org/assignments/media-types/application/font-woff">registered</a>
   * with the IANA.
   *
   * @since 17.0
   */
   ...
   * with the IANA, but it is specified by OASIS in the
  ...
  /**
   * Returns the media subtype. For example, {@code "plain"} in
   * {@code "text/plain"}.
   */
   ...
   * specified.
   *
   * @throws IllegalStateException       if multiple charset values have been set
   *                                     for this media type
   * @throws IllegalCharsetNameException if a charset value is present, but
   *                                     illegal
   * @throws UnsupportedCharsetException if a charset value is present, but no
   *                                     support is available in this instance of
  ...
  /**
   * Returns a new instance with the same type and subtype as this instance, but
  ...
  /**
   * <em>Replaces</em> all parameters with the given attribute with a single
   * parameter with the given value. If multiple parameters with the same
   * attributes are necessary use {@link #withParameters}. Prefer
   * {@link #withCharset} for setting the {@code charset} parameter when using a
   * {@link Charset} object.
   *
   * @throws IllegalArgumentException if either {@code attribute} or {@code value}
   *                                  is invalid
   */
  ...
  /**
   * Returns a new instance with the same type and subtype as this instance, with
   * the {@code charset} parameter set to the {@link Charset#name name} of the
   * given charset. Only one {@code charset} parameter will be present on the new
   * instance regardless of the number set on this one.
   *
   * <p>
   * If a charset must be specified that is not supported on this JVM (and thus is
   * not representable as a {@link Charset} instance, use {@link #withParameter}.
   */
   ...
   * <a href="http://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html">the HTTP
   * Accept header</a>) given by the argument according to three criteria:
   *
   * <ol>
   * <li>The type of the argument is the wildcard or equal to the type of this
   * instance.
   * <li>The subtype of the argument is the wildcard or equal to the subtype of
   * this instance.
   * <li>All of the parameters present in the argument are present in this
   * instance.
   * </ol>
   *
   * <p>
   ...
   * times within a media type this method does not consider the number of
   * occurrences of a parameter. For example, {@code "text/plain; charset=UTF-8"}
   * satisfies {@code "text/plain; charset=UTF-8; charset=UTF-8"}.
   */
   ...
   *
   * @throws IllegalArgumentException if type or subtype is invalid or if a
   *                                  wildcard is used for the type, but not the
   *                                  subtype.
   */
  ...
  /**
   * Returns the string representation of this media type in the format described
   * in <a href="http://www.ietf.org/rfc/rfc2045.txt">RFC 2045</a>.
   */
  @Override
