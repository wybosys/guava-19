...
import com.google.v19.common.base.Splitter;
import com.google.v19.common.collect.ImmutableList;
import com.google.v19.thirdparty.publicsuffix.PublicSuffixPatterns;
 ...
 *
 * <p>
 * One common use of this class is to determine whether a given string is likely
 * to represent an addressable domain on the web -- that is, for a candidate
 * string {@code "xxx"}, might browsing to {@code "http://xxx/"} result in a
 * webpage being displayed? In the past, this test was frequently done by
 * determining whether the domain ended with a {@linkplain #isPublicSuffix()
 * public suffix} but was not itself a public suffix. However, this test is no
 * longer accurate. There are many domains which are both public suffixes and
 * addressable as hosts; {@code "uk.com"} is one example. As a result, the only
 * useful test to determine if a domain is a plausible web host is
 * {@link #hasPublicSuffix()}. This will return {@code true} for many domains
 ...
 * </ol>
 * <p>
 * The normalized values will be returned from {@link #toString()} and
...
public final class InternetDomainName {
  ...
  /**
   * Value of {@link #publicSuffixIndex} which indicates that no public suffix was
   * found.
   */
  ...
  /**
   * Maximum parts (labels) in a domain name. This value arises from the 255-octet
   * limit described in <a href="http://www.ietf.org/rfc/rfc2181.txt">RFC 2181</a>
   * part 11 with the fact that the encoding of each part occupies at least two
   * bytes (dot plus label externally, length byte plus label internally). Thus,
   * if all labels have the minimum size of one byte, 127 of them will fit.
   */
  private static final int MAX_PARTS = 127;

  /**
   * Maximum length of a full domain name, including separators, and leaving room
   * for the root label. See <a href="http://www.ietf.org/rfc/rfc2181.txt">RFC
   * 2181</a> part 11.
   */
   ...
   * example, for the domain name {@code www.google.co.uk}, the value would be 2
   * (the index of the {@code co} part). The value is negative (specifically,
   * {@link #NO_PUBLIC_SUFFIX_FOUND}) if no public suffix was found.
   */
   ...
   * found. Note that the value defined as the "public suffix" may not be a public
   * suffix according to {@link #isPublicSuffix()} if the domain ends with an
   * excluded domain pattern such as {@code "nhs.uk"}.
   */
   ...
   * validation against <a href="http://www.ietf.org/rfc/rfc1035.txt">RFC 1035</a>
   * is relaxed in the following ways:
   * <ul>
  ...
  private static boolean validatePart(String part, boolean isFinalPart) {
    ...
    /*
     * GWT claims to support java.lang.Character's char-classification methods, but
     * it actually only works for ASCII. So for now, assume any non-ASCII characters
     * are valid. The only place this seems to be documented is here:
     ...
     * relevant RFCs) domain parts other than the last may begin with a digit (for
     * example, "3com.com"). It's important to disallow an initial digit in the last
     * part; it's the only thing that stops an IPv4 numeric address like 127.0.0.1
     * from looking like a valid domain name.
     */
   ...
   * defined by the Mozilla Foundation's <a href="http://publicsuffix.org/">Public
   * Suffix List</a> (PSL). A public suffix is one under which Internet users can
   * directly register names, such as {@code com}, {@code co.uk} or
   * {@code pvt.k12.wy.us}. Examples of domain names that are <i>not</i> public
   * suffixes include {@code google}, {@code
   * google.com} and {@code foo.co.uk}.
   *
   * @return {@code true} if this domain name appears exactly on the public suffix
   *         list
   * @since 6.0
   */
   ...
   * {@code com}, but not for {@code google} or {@code google.foo}. This is the
   * recommended method for determining whether a domain is potentially an
   * addressable host.
   *
   * @since 6.0
   */
  ...
  /**
   * Indicates whether this domain name ends in a {@linkplain #isPublicSuffix()
   * public suffix}, while not being a public suffix itself. For example, returns
   * {@code true} for {@code www.google.com}, {@code foo.co.uk} and
   * {@code bar.ca.us}, but not for {@code google}, {@code com}, or {@code
   * google.foo}.
   *
   * <p>
   * <b>Warning:</b> a {@code false} result from this method does not imply that
   * the domain does not represent an addressable host, as many public suffixes
   * are also addressable hosts. Use {@link #hasPublicSuffix()} for that test.
   *
   * <p>
   * This method can be used to determine whether it will probably be possible to
   * set cookies on the domain, though even that depends on individual browsers'
   ...
   *
   * <p>
   * <b>Warning:</b> A {@code true} result from this method does not imply that
   * the domain is at the highest level which is addressable as a host, as many
   * public suffixes are also addressable hosts. For example, the domain
   * {@code bar.uk.com} has a public suffix of {@code uk.com}, so it would return
   * {@code true} from this method. But {@code uk.com} is itself an addressable
   * host.
   *
   * <p>
   * This method can be used to determine whether a domain is probably the highest
   * level for which cookies may be set, though even that depends on individual
   * browsers' implementations of cookie controls. See
   * <a href="http://www.ietf.org/rfc/rfc2109.txt">RFC 2109</a> for details.
   *
   * @since 6.0
   */
   ...
   * suffix. For example, for {@code x.adwords.google.co.uk} it returns
   ...
   *
   * <p>
   * This method should not be used to determine the topmost parent domain which
   * is addressable as a host, as many public suffixes are also addressable hosts.
   * For example, the domain {@code foo.bar.uk.com} has a public suffix of
   * {@code uk.com}, so it would return {@code bar.uk.com} from this method. But
   * {@code uk.com} is itself an addressable host.
   *
   * <p>
   * This method can be used to determine the probable highest level parent domain
   * for which cookies may be set, though even that depends on individual
   * browsers' implementations of cookie controls.
   *
   * @throws IllegalStateException if this domain does not end with a public
   *                               suffix
   * @since 6.0
   */
  ...
  /**
   * Returns an {@code InternetDomainName} that is the immediate ancestor of this
   * one; that is, the current domain with the leftmost part removed. For example,
   * the parent of {@code www.google.com} is {@code google.com}.
   *
   ...
   * lenient validation. Specifically, validation against
   * <a href="http://www.ietf.org/rfc/rfc3490.txt">RFC 3490</a>
  ...
  /**
   * Equality testing is based on the text supplied by the caller, after
   * normalization as described in the class documentation. For example, a
   * non-ASCII Unicode domain name and the Punycode version of the same domain
   * name would not be considered equal.
   *
   */
  @Override
