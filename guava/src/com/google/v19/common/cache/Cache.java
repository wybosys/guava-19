 ...
 * added using {@link #get(Object, Callable)} or {@link #put(Object, Object)},
...
public interface Cache<K, V> {
   ...
   * value from {@code valueLoader} if necessary. No observable state associated
   * with this cache is modified until loading completes. This method provides a
   * simple substitute for the conventional "if cached, return; otherwise create,
   * cache and return" pattern.
   *
   * <p>
   * <b>Warning:</b> as with {@link CacheLoader#load}, {@code valueLoader} <b>must
   * not</b> return {@code null}; it may either return a non-null value or throw
   * an exception.
   *
  ...
  /**
   * Copies all of the mappings from the specified map to the cache. The effect of
   ...
   *
   * <p>
   * Iterators from the returned map are at least <i>weakly consistent</i>: they
   * are safe for concurrent use, but if the cache is modified (including by
   * eviction) after the iterator is created, it is undefined which of the changes
