...
package com.google.v19.common.cache;
...
import static com.google.v19.common.cache.CacheBuilder.NULL_TICKER;
import static com.google.v19.common.cache.CacheBuilder.UNSET_INT;
...
import com.google.v19.common.annotations.VisibleForTesting;
...
import com.google.v19.common.cache.AbstractCache.SimpleStatsCounter;
import com.google.v19.common.cache.AbstractCache.StatsCounter;
import com.google.v19.common.cache.CacheBuilder.NullListener;
import com.google.v19.common.cache.CacheBuilder.OneWeigher;
import com.google.v19.common.cache.CacheLoader.InvalidCacheLoadException;
import com.google.v19.common.cache.CacheLoader.UnsupportedLoadingOperationException;
import com.google.v19.common.collect.AbstractSequentialIterator;
import com.google.v19.common.collect.ImmutableMap;
import com.google.v19.common.collect.ImmutableSet;
import com.google.v19.common.collect.Iterators;
import com.google.v19.common.collect.Maps;
import com.google.v19.common.collect.Sets;
import com.google.v19.common.primitives.Ints;
import com.google.v19.common.util.concurrent.ExecutionError;
import com.google.v19.common.util.concurrent.Futures;
import com.google.v19.common.util.concurrent.ListenableFuture;
import com.google.v19.common.util.concurrent.SettableFuture;
import com.google.v19.common.util.concurrent.UncheckedExecutionException;
import com.google.v19.common.util.concurrent.Uninterruptibles;
 ...
 *
 * @author Charles Fry
 * @author Bob Lee ({@code com.google.v19.common.collect.MapMaker})
...
class LocalCache<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V> {
   ...
   * itself is a concurrently readable hash table. The map supports non-blocking
   * reads and concurrent writes across different segments.
   *
   ...
   * with the map. The ordering of writes to a segment is sequentially consistent.
   * An update to the map and recording of reads may not be immediately reflected
   * on the algorithm's data structures. These structures are guarded by a lock
   * and operations are applied in batches to avoid lock contention. The penalty
   * of applying the batches is spread across threads so that the amortized cost
   * is slightly higher than performing just the operation without enforcing the
   * capacity constraint.
   *
   ...
   * complexity. The initial LRU implementation operates per-segment rather than
   * globally for increased implementation simplicity. We expect the cache hit
   * rate to be similar to that of a global LRU algorithm.
   */
   ...
   * in the same segment from also ending up in the same bucket.
   */
  ...
  // TODO(fry): define a new type which creates event objects and automates the
  // clear logic
   ...
   * stats counters which must be aggregated to obtain a global stats view.
   */
  ...
  enum Strength {
     ...
     * values referenced at this strength. This strategy will be used unless the
     * user explicitly specifies an alternate strategy.
     */
  ...
  interface ValueReference<K, V> {
     ...
     * this value reference is independent of any entry.
     */
    @Nullable
    ...
    /**
     * Notifify pending loads that a new value was set. This is only relevant to
     * loading value references.
     */
    void notifyNewValue(@Nullable V newValue);

    /**
     * Returns true if a new value is currently loading, regardless of whether or
     * not there is an existing value. It is assumed that the return value of this
     * method is constant for any given ValueReference instance.
     */
    boolean isLoading();
   ...
   *
   * Invalid: - Expired: time expired (key/value may still be set) - Collected:
   * key/value was partially collected, but not yet cleaned up - Unset: marked as
   * unset, awaiting cleanup or reuse
   */
  interface ReferenceEntry<K, V> {
     ...
     * doubly-linked list. New entries are added at the tail of the list at write
     * time; stale entries are expired from the head of the list.
     */
     ...
     * a doubly-linked list. New entries are added at the tail of the list at write
     * time and stale entries are expired from the head of the list.
     */
   ...
   * Java had mixins! To maintain this code, make a change for the strong
   * reference type. Then, cut and paste, and replace "Strong" with "Soft" or
   * "Weak" within the pasted text. The primary difference is that strong entries
   * store the key reference directly while soft and weak entries delegate to
   ...
   * partially-collected, loading, or expired. Unlike {@link Segment#getLiveValue}
   * this method does not attempt to cleanup stale entries. As such it should only
   ...
   * expiration, eviction, or eligibility for garbage collection. This should be
   * called every time expireEntries or evictEntry is called (once the lock is
   * released).
   */
  ...
  static class Segment<K, V> extends ReentrantLock {
     ...
     * into this class. It will require more memory but will reduce indirection.
     */
     ...
     * this field is always {@code (int) (capacity * 0.75)}.)
     */
    int threshold;
     ...
     * the access list's ordering. It is drained as a batch operation when either
     * the DRAIN_THRESHOLD is crossed or a write occurs on the segment.
     */
    final Queue<ReferenceEntry<K, V>> recencyQueue;

    /**
     * A counter of the number of reads since the last write, used to drain queues
     * on a small fraction of read operations.
     */
     ...
     * are added to the tail of the queue on access (note that writes count as
     * accesses).
     */
    @GuardedBy("this")
    ...
    ListenableFuture<V> loadAsync(final K key, final int hash, final LoadingValueReference<K, V> loadingValueReference,
     ...
     * already doing so. Returns the newly refreshed value associated with
     * {@code key} if it was refreshed inline, or {@code null} if another thread is
    ...
    @GuardedBy("this")
    @Nullable
    ReferenceEntry<K, V> removeValueFromChain(ReferenceEntry<K, V> first, ReferenceEntry<K, V> entry, @Nullable K key,
        int hash, ValueReference<K, V> valueReference, RemovalCause cause) {
   ...
   * which are in the map are also in this queue, and that all elements not in the
   * queue are not in the map.
   *
   * <p>
   * The benefits of creating our own queue are that (1) we can replace elements
   * in the middle of the queue as part of copyWriteEntry, and (2) the contains
   * method is highly optimized for the current model.
   */
   ...
   * integrated with {@code ReferenceEntry}, upon which it reliese to perform its
   * linking.
   *
   * <p>
   * Note that this entire implementation makes the assumption that all elements
  ...
  public boolean isEmpty() {
     ...
     * Method containsValue() uses similar constructions for stability checks.
     */
    long sum = 0L;
  ...
  abstract class AbstractCacheSet<T> extends AbstractSet<T> {
    ...
    // super.toArray() may misbehave if size() is inaccurate, at least on old
    // versions of Android.
  ...
  /**
   * Serializes the configuration of a LocalCache, reconsitituting it as a Cache
   * using CacheBuilder upon deserialization. An instance of this class is fit for
   * use by the writeReplace of LocalManualCache.
   *
  ...
  static class ManualSerializationProxy<K, V> extends ForwardingCache<K, V> implements Serializable {
    ...
    private ManualSerializationProxy(Strength keyStrength, Strength valueStrength, Equivalence<Object> keyEquivalence,
        Equivalence<Object> valueEquivalence, long expireAfterWriteNanos, long expireAfterAccessNanos, long maxWeight,
        Weigher<K, V> weigher, int concurrencyLevel, RemovalListener<? super K, ? super V> removalListener,
    ...
    CacheBuilder<K, V> recreateCacheBuilder() {
      CacheBuilder<K, V> builder = CacheBuilder.newBuilder().setKeyStrength(keyStrength).setValueStrength(valueStrength)
          .keyEquivalence(keyEquivalence).valueEquivalence(valueEquivalence).concurrencyLevel(concurrencyLevel)
  ...
  /**
   * Serializes the configuration of a LocalCache, reconsitituting it as an
