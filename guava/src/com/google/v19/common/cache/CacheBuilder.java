...
import com.google.v19.common.base.Suppliers;
...
import com.google.v19.common.cache.LocalCache.Strength;
...
public final class CacheBuilder<K, V> {
   ...
   *
   * <p>
   * By default, the cache uses {@link Equivalence#identity} to determine value
   * equality when {@link #weakValues} or {@link #softValues} is specified, and
   * {@link Equivalence#equals()} otherwise.
   */
  @GwtIncompatible("To be supported")
   ...
   * at a time, but since read operations and cache loading computations can
   * proceed concurrently, this still yields higher concurrency than full
   * synchronization.
   *
   * <p>
   * Defaults to 4. <b>Note:</b>The default may change in the future. If you care
   * about this value, you should always choose it explicitly.
   *
   * <p>
   ...
   * segment lock is taken once for each explicit write, and twice for each cache
   * loading computation (once prior to loading the new value, and once after
   * loading completes). Much internal cache management is performed at the
   * segment granularity. For example, access queues and write queues are kept per
   * segment when they are required by the selected eviction algorithm. As such,
   * when writing unit tests it is not uncommon to specify
   ...
   * determined using the {@link Weigher} specified with {@link #weigher}, and use
   * of this method requires a corresponding call to {@link #weigher} prior to
   * calling {@link #build}.
   *
   * <p>
   * Note that the cache <b>may evict an entry before this limit is exceeded</b>.
   * As the cache size grows close to the maximum, the cache evicts entries that
   * are less likely to be used again. For example, the cache may evict an entry
   * because it hasn't been used recently or very often.
   *
   * <p>
   * When {@code weight} is zero, elements will be evicted immediately after being
   ...
   * capacity; it has no effect on selecting which entry should be evicted next.
   *
   * <p>
   ...
   * {@link #build}. Weights are measured and recorded when entries are inserted
   * into the cache, and are thus effectively static during the lifetime of a
   * cache entry.
   *
   * <p>
   ...
   * configuring a {@code CacheBuilder} and building your {@link Cache} all in a
   * single statement.
   *
   * <p>
   ...
   * with the weigher, you will likely experience a {@link ClassCastException} at
   ...
   * a {@link WeakReference} (by default, strong references are used).
   *
   * <p>
   ...
   * {@link Cache#size}, but will never be visible to read or write operations;
   * such entries are cleaned up as part of the routine maintenance described in
   * the class javadoc.
   *
  ...
  /**
   * Specifies that each value (not key) stored in the cache should be wrapped in
   * a {@link SoftReference} (by default, strong references are used).
   ...
   * references. You should only use this method if you are well familiar with the
   ...
   * otherwise-specificed maximum size or weight. This can be useful in testing,
   ...
   * visible to read or write operations. Expired entries are cleaned up as part
   * of the routine maintenance described in the class javadoc.
   *
   ...
   * cache read and write operations (including {@code Cache.asMap().get(Object)}
   * and {@code Cache.asMap().put(K, V)}), but not by operations on the
   ...
   *
   * <p>
   * As the default implementation of {@link CacheLoader#reload} is synchronous,
   * it is recommended that users of this method override
   * {@link CacheLoader#reload} with an asynchronous implementation; otherwise
   * refreshes will be performed during unrelated cache read and write operations.
   *
   * <p>
   * Currently automatic refreshes are performed when the first stale request for
   * an entry occurs. The request triggering refresh will make a blocking call to
   * {@link CacheLoader#reload} and immediately return the new value if the
   * returned future is complete, and the old value otherwise.
   *
   * <p>
   ...
   *
   * @param duration the length of time after an entry is created that it should
   *                 be considered stale, and thus eligible for refresh
   * @param unit     the unit that {@code duration} is expressed in
   * @throws IllegalArgumentException if {@code duration} is negative
   ...
   *
   * <p>
   * The primary intent of this method is to facilitate testing of caches with a
   * fake or mock time source.
   *
   ...
   * builder will invoke this listener as part of the routine maintenance
   * described in the class documentation above.
   *
   * <p>
   * <b>Warning:</b> after invoking this method, do not continue to use
   * <i>this</i> cache builder reference; instead use the reference this method
   * <i>returns</i>. At runtime, these point to the same instance, but only the
   * returned reference has the correct generic type information so as to ensure
   ...
   * building your cache in a single statement. Failure to heed this advice can
   * result in a {@link ClassCastException} being thrown by a cache operation at
   * some <i>undefined</i> point in the future.
   *
   * <p>
   ...
   * cache. Without this {@link Cache#stats} will return zero for all statistics.
   * Note that recording stats requires bookkeeping to be performed with each
  ...
  /**
   * Builds a cache, which either returns an already-loaded value for a given key
   * or atomically computes or retrieves it using the supplied
   ...
   * value. Note that multiple threads can concurrently load values for distinct
   * keys.
   *
   * <p>
   * This method does not alter the state of this {@code CacheBuilder} instance,
   ...
   *
   * <p>
   * Consider {@link #build(CacheLoader)} instead, if it is feasible to implement
   * a {@code CacheLoader}.
   *
   * <p>
   * This method does not alter the state of this {@code CacheBuilder} instance,
