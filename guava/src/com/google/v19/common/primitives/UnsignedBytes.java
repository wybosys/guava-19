...
public final class UnsignedBytes {
  ...
  static class LexicographicalComparatorHolder {
    ...
    enum UnsafeComparator implements Comparator<byte[]> {
       ...
       * fields is the fastest (more than twice as fast as the Java implementation, vs
       * ~1.5x with non-final static fields, on x86_32) under the Hotspot server
       * compiler. The reason is obviously that the non-final fields need to be
       * reloaded inside the loop.
       *
       * And, no, defining (final or not) local variables out of the loop still isn't
       * as good because the null check on the theUnsafe object remains inside the
       * loop and BYTE_ARRAY_BASE_OFFSET doesn't get constant-folded.
       *
      ...
      public int compare(byte[] left, byte[] right) {
        ...
        /*
         * Compare 8 bytes at a time. Benchmarking shows comparing 8 bytes at a time is
         * no slower than comparing 4 bytes at a time even on 32-bit. On the other hand,
         * it is substantially faster on 64-bit.
         */
        for (int i = 0; i < minWords * Longs.BYTES; i += Longs.BYTES) {
          ...
          if (lw != rw) {
             ...
             * significant nonzero bit, and zeroing out the first three bits of L.nTZ gives
             * us the shift to get that least significant nonzero byte.
             */
