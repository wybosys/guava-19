...
abstract class AggregateFuture<InputT, OutputT> extends AbstractFuture.TrustedFuture<OutputT> {
  ...
  // TODO(cpovirk): Use maybePropagateCancellation() if the performance is OK and
  // the code is clean.
  @Override
  public final boolean cancel(boolean mayInterruptIfRunning) {
    // Must get a reference to the futures before we cancel, as they'll be cleared
    // out.
    RunningState localRunningState = runningState;
    ImmutableCollection<? extends ListenableFuture<? extends InputT>> futures = (localRunningState != null)
  ...
  abstract class RunningState extends AggregateFutureState implements Runnable {
    ...
    /**
     * The "real" initialization; we can't put this in the constructor because, in
     * the case where futures are already complete, we would not initialize the
     * subclass before calling {@link #handleOneInputDone}. As this is called after
     * the subclass is constructed, we're guaranteed to have properly initialized
     * the subclass.
     */
    private void init() {
     ...
     * {@link #allMustSucceed} is {@code true}, the throwable did not cause this
     * future to fail, and it is the first time we've seen that particular
     * Throwable.
     */
    ...
    private void processCompleted() {
      // Collect the values if (a) our output requires collecting them and (b) we
      // haven't been
      // collecting them as we go. (We've collected them as we go only if we needed to
      // fail fast)
    ...
    /**
     * Listeners implicitly keep a reference to {@link RunningState} as they're
     * inner classes, so we free resources here as well for the allMustSucceed=true
     * case (i.e. when a future fails, we immediately release resources we no longer
     * need); additionally, the future will release its reference to
     * {@link RunningState}, which should free all associated memory when all the
     ...
     *
     * <p>
     * If {@code allMustSucceed} is true, called as each future completes;
  ...
  /**
   * Adds the chain to the seen set, and returns whether all the chain was new to
   * us.
   */
  private static boolean addCausalChain(Set<Throwable> seen, Throwable t) {
    for (; t != null; t = t.getCause()) {
      ...
      if (!firstTimeSeen) {
         ...
         * (There's one case where this isn't true, but we ignore it: If we record an
         * exception, then someone calls initCause() on it, and then we examine it
         * again, we'll conclude that we've seen the whole chain before when it fact we
         * haven't. But this should be rare.)
         */
        return false;
      }
    }
