...
/**
 * A special purpose queue/executor that executes listener callbacks serially on
 * a configured executor. Each callback task can be enqueued and executed as
 * separate phases.
 * 
 * <p>
 * This class is very similar to {@link SerializingExecutor} with the exception
 * that tasks can be enqueued without necessarily executing immediately.
 */
final class ListenerCallQueue<L> implements Runnable {
  ...
  /**
   * Executes all listeners {@linkplain #add added} prior to this call, serially
   * and in order.
   */
  void execute() {
    ...
    if (scheduleTaskRunner) {
      try {
        ...
        // reset state in case of an error so that later calls to execute will actually
        // do something
        synchronized (this) {
