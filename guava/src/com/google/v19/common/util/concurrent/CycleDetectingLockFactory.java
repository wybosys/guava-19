...
import com.google.v19.common.collect.MapMaker;
...
public class CycleDetectingLockFactory {
  ...
  public interface Policy {
    ...
    /**
     * Called when a potential deadlock is encountered. Implementations can throw
     * the given {@code exception} and/or execute other desired logic.
     * <p>
  ...
  public enum Policies implements Policy {
    /**
     * When potential deadlock is detected, this policy results in the throwing of
     * the {@code PotentialDeadlockException} indicating the potential deadlock,
     * which includes stack traces illustrating the cycle in lock acquisition order.
     */
    THROW {
      @Override
     ...
     * {@link Level#SEVERE} message indicating the potential deadlock, which
     * includes stack traces illustrating the cycle in lock acquisition order.
     */
    WARN {
      @Override
      public void handlePotentialDeadlock(PotentialDeadlockException e) {
     ...
     * lock implementations provided by the JDK, and is provided to allow
     * applications to easily parameterize when cycle detection is enabled.
     * <p>
   ...
   * {@code lockName} is used in the warning or exception output to help identify
   * the locks involved in the detected deadlock.
   */
   ...
   * values to a corresponding LockGraphNode, with the {@code allowedPriorLocks}
   * and {@code disallowedPriorLocks} prepopulated with nodes according to the
   * natural ordering of the associated Enum values.
   */
  @VisibleForTesting
   ...
   * {@code Enum} whose values each correspond to a lock type. The order in which
   * the values are declared dictates the allowed order of lock acquisition. In
   * other words, locks corresponding to smaller values of {@link Enum#ordinal()}
   * should only be acquired before locks with larger ordinals. Example:
   *
   * <pre>
   *    {@code
   * enum MyLockOrder {
   ...
   * lock acquisition is defined by the {@code Policy} of the factory that created
   * it.
   *
   * <p>
   ...
   * calls to the same factory, attempting to acquire multiple locks with the same
   * Enum value (within the same thread) will result in an IllegalStateException
   * regardless of the factory's policy. For example:
   *
   * <pre>
   *    {@code
  ...
  public static final class WithExplicitOrdering<E extends Enum<E>> extends CycleDetectingLockFactory {
     ...
     * values returned by {@link Enum#getDeclaringClass()} and {@link Enum#name()}
     * are used to describe the lock in warning or exception output.
     *
  ...
  /**
   * Tracks the currently acquired locks for each Thread, kept up to date by calls
   * to {@link #aboutToAcquire(CycleDetectingLock)} and
   ...
   * specific lock acquisition ordering. The top of the stack trace is truncated
   * such that it starts with the acquisition of the lock in question, e.g.
   *
   * <pre>
   ...
   * includes a causal chain of {@code ExampleStackTrace} instances to illustrate
   * the cycle, e.g.
   *
   * <pre>
  ...
  private static class LockGraphNode {
    ...
    /**
     * The map tracking the locks that are known to be acquired before this lock,
     * each associated with an example stack trace. Locks are weakly keyed to allow
     * proper garbage collection when they are no longer referenced.
     */
     ...
     * {@code preAcquireLocks} map, for the case in which it is safe to acquire
     * {@code this} after the {@code acquiredLock}, or in the
     ...
     * {@code allowedPriorLocks} to find a path between {@code this} and the
     * specified {@code lock}.
     *
   ...
   * clause after any attempt to change the lock state, including both lock and
   * unlock attempts. Failure to do so can result in corrupting the acquireLocks
   * set.
   */
