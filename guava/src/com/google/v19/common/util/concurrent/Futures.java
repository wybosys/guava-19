...
package com.google.v19.common.util.concurrent;

import static com.google.v19.common.base.Preconditions.checkNotNull;
import static com.google.v19.common.util.concurrent.MoreExecutors.directExecutor;
import static com.google.v19.common.util.concurrent.Platform.isInstanceOfThrowableClass;
import static com.google.v19.common.util.concurrent.Uninterruptibles.getUninterruptibly;

import com.google.v19.common.annotations.Beta;
import com.google.v19.common.annotations.GwtCompatible;
import com.google.v19.common.annotations.GwtIncompatible;
import com.google.v19.common.base.Function;
import com.google.v19.common.base.Optional;
import com.google.v19.common.base.Preconditions;
import com.google.v19.common.collect.ImmutableCollection;
import com.google.v19.common.collect.ImmutableList;
import com.google.v19.common.collect.Lists;
import com.google.v19.common.collect.Queues;
 ...
 *
 * <p>
 * Many of these methods use the {@link ListenableFuture} API; consult the Guava
...
public final class Futures extends GwtFuturesCatchingSpecialization {
  ...
  // Many of the utilities in this class (transform, withFallback, withTimeout,
  // asList, combine)
  // have two requirements that significantly complicate their design.
  // 1. Cancellation should propagate from the returned future to the input
  // future(s).
  // 2. The returned futures shouldn't unnecessarily 'pin' their inputs after
  // completion.
  //
  // A consequence of these requirements is that the delegate futures cannot be
  // stored in
  // final fields.
  //
  // For simplicity the rest of this description will discuss Futures.catching
  // since it is the
  // simplest instance, though very similar descriptions apply to many other
  // classes in this file.
  //
  // In the constructor of AbstractCatchingFuture, the delegate future is assigned
  // to a field
  // 'inputFuture'. That field is non-final and non-volatile. There are 2 places
  // where the
  // 'inputFuture' field is read and where we will have to consider visibility of
  // the write
  ...
  // invocation of the
  // listener. Notably, this means that 'volatile' is unnecessary to make
  // 'inputFuture' visible
  // to the listener.
  //
  // 2. In done() where we may propagate cancellation to the input. In this case
  // it is _not_ fine.
  // There is currently nothing that enforces that the write to inputFuture in the
  // constructor is
  // visible to done(). This is because there is no happens before edge between
  // the write and a
  // (hypothetical) unsafe read by our caller. Note: adding 'volatile' does not
  // fix this issue,
  // it would just add an edge such that if done() observed non-null, then it
  // would also
  // definitely observe all earlier writes, but we still have no guarantee that
  // done() would see
  // the inital write (just stronger guarantees if it does).
  //
  // See:
  // http://cs.oswego.edu/pipermail/concurrency-interest/2015-January/013800.html
  // For a (long) discussion about this specific issue and the general futility of
  // life.
  //
  // For the time being we are OK with the problem discussed above since it
  // requires a caller to
  // introduce a very specific kind of data-race. And given the other operations
  // performed by these
  // methods that involve volatile read/write operations, in practice there is no
  // issue. Also, the
  // way in such a visibility issue would surface is most likely as a failure of
  // cancel() to
  // propagate to the input. Cancellation propagation is fundamentally racy so
  // this is fine.
  //
  // Future versions of the JMM may revise safe construction semantics in such a
  // way that we can
  // safely publish these objects and we won't need this whole discussion.
  // TODO(user,lukes): consider adding volatile to all these fields since in
  // current known JVMs
  // that should resolve the issue. This comes at the cost of adding more write
  // barriers to the
  // implementations.
  ...
  /**
   * Creates a {@link CheckedFuture} out of a normal {@link ListenableFuture} and
   * a {@link Function} that maps from {@link Exception} instances into the
   ...
   * {@code CheckedFuture} API. Additionally, {@code
   ...
   * {@link ExecutionException}. See {@link Future#get()} for details on the
   * exceptions thrown.
   *
  ...
  private abstract static class ImmediateFuture<V> implements ListenableFuture<V> {
    ...
    public void addListener(Runnable listener, Executor executor) {
      ...
      try {
        ...
        log.log(Level.SEVERE, "RuntimeException while executing runnable " + listener + " with executor " + executor,
            e);
      }
    }
  ...
  @GwtIncompatible("TODO")
  private static class ImmediateSuccessfulCheckedFuture<V, X extends Exception> extends ImmediateFuture<V>
  ...
  @GwtIncompatible("TODO")
  private static class ImmediateFailedCheckedFuture<V, X extends Exception> extends ImmediateFuture<V>
      implements CheckedFuture<V, X> {
   ...
   * canceled or timed out and its {@code isDone()} method always returns
   * {@code true}.
   */
  @CheckReturnValue
   ...
   * ExecutionException}, and calling {@code checkedGet()} will throw the provided
   * exception itself.
   */
  @GwtIncompatible("TODO")
  @CheckReturnValue
   ...
   * invoked until the primary input has failed, so if the primary input succeeds,
   * it is never invoked. If, during the invocation of {@code
   ...
   * directExecutor}, a dangerous choice in some cases. See the discussion in the
   * {@link ListenableFuture#addListener ListenableFuture.addListener}
   * documentation. The documentation's warnings about "lightweight listeners"
   * refer here to the work done during {@code FutureFallback.create}, not to any
   ...
   *
   * <p>
   * When selecting an executor, note that {@code directExecutor} is dangerous in
   * some cases. See the discussion in the {@link ListenableFuture#addListener
   * ListenableFuture.addListener} documentation. The documentation's warnings
   * about "lightweight listeners" refer here to the work done during
   * {@code FutureFallback.create}, not to any work done to complete the returned
   * {@code Future}.
   *
   * @param input    the primary input {@code Future}
   * @param fallback the {@link FutureFallback} implementation to be called if
   ...
   * @since 14.0
   * @deprecated Use
   *             {@link #catchingAsync(ListenableFuture, Class, AsyncFunction, Executor)
   *             catchingAsync(input, Throwable.class,
   *             fallbackImplementedAsAnAsyncFunction, executor)}, usually
   *             replacing {@code Throwable.class} with the specific type you want
   *             to handle. This method will be removed in Guava release 20.0.
   */
  @Deprecated
  @CheckReturnValue
  public static <V> ListenableFuture<V> withFallback(ListenableFuture<? extends V> input,
  ...
  /**
   * Returns a {@code Future} whose result is taken from the given primary
   * {@code input} or, if the primary input fails with the given
   * {@code exceptionType}, from the result provided by the {@code fallback}.
   * {@link Function#apply} is not invoked until the primary input has failed, so
   * if the primary input succeeds, it is never invoked. If, during the invocation
   * of {@code
   * fallback}, an exception is thrown, this exception is used as the result of
   * the output {@code
   * Future}.
   ...
   * {@code directExecutor}, a dangerous choice in some cases. See the discussion
   * in the {@link ListenableFuture#addListener ListenableFuture.addListener}
   * documentation. The documentation's warnings about "lightweight listeners"
   * refer here to the work done during {@code Function.apply}.
   *
   * @param input         the primary input {@code Future}
   * @param exceptionType the exception type that triggers use of
   *                      {@code fallback}. To avoid hiding bugs and other
   *                      unrecoverable errors, callers should prefer more
   *                      specific types, avoiding {@code Throwable.class} in
   *                      particular.
   * @param fallback      the {@link Function} implementation to be called if
   *                      {@code input} fails with the expected exception type
   * @since 19.0
   */
   ...
   * {@code input} or, if the primary input fails with the given
   * {@code exceptionType}, from the result provided by the {@code fallback}.
   * {@link AsyncFunction#apply} is not invoked until the primary input has
   * failed, so if the primary input succeeds, it is never invoked. If, during the
   * invocation of {@code fallback}, an exception is thrown, this exception is
   * used as the result of the output {@code Future}.
   *
   * <p>
   * Usage examples:
   *
   ...
   *                      specific types, avoiding {@code Throwable.class} in
   *                      particular.
   * @param fallback      the {@link AsyncFunction} implementation to be called if
   *                      {@code input} fails with the expected exception type
   ...
   * ListenableFuture.addListener} documentation. The documentation's warnings
   * about "lightweight listeners" refer here to the work done during
   * {@code AsyncFunction.apply}, not to any work done to complete the returned
   * {@code Future}.
   *
   * @param input         the primary input {@code Future}
   * @param exceptionType the exception type that triggers use of
  ...
  static final class AsyncCatchingFuture<V, X extends Throwable>
    ...
    void doFallback(AsyncFunction<? super X, ? extends V> fallback, X cause) throws Exception {
      ...
      checkNotNull(replacement,
          "AsyncFunction.apply returned null instead of a Future. " + "Did you mean to return immediateFuture(null)?");
      setFuture(replacement);
    }
  }

  /**
   * A {@link AbstractCatchingFuture} that delegates to a {@link Function} and
   * {@link #set(Object)} to implement {@link #doFallback}
   */
   ...
   * {@link TimeoutException} wrapped in an {@link ExecutionException}) if the
   * specified duration expires. The delegate future is interrupted and cancelled
   * if it times out.
   */
  private static final class TimeoutFuture<V> extends AbstractFuture.TrustedFuture<V> {
    ...
    // The initial write to delegateRef is made definitely visible via the semantics
    // of
    // addListener/SES.schedule. The later racy write in cancel() is not guaranteed
    // to be
    // observed, however that is fine since the correctness is based on the atomic
    // state in
    // our base class.
    // The initial write to timer is never definitely visible to Fire.run since it
    // is assigned
    // after SES.schedule is called. Therefore Fire.run has to check for null.
    // However, it
    // should be visible if Fire.run is called by delegate.addListener since
    // addListener is
    // called after the assignment to timer, and importantly this is the main
    // situation in which
    // we need to be able to see the write.
    // 2. visibility of the writes to cancel
    // Since these fields are non-final that means that TimeoutFuture is not being
    // 'safely
    // published', thus a motivated caller may be able to expose the reference to
    // another thread
    // that would then call cancel() and be unable to cancel the delegate.
    // There are a number of ways to solve this, none of which are very pretty, and
    // it is
    // currently believed to be a purely theoretical problem (since the other
    // actions should
    ...
    private static final class Fire<V> implements Runnable {
      ...
      public void run() {
         ...
         * reference to it. Otherwise, we'll pin it (and its result) in memory until the
         * timeout task is GCed. (The need to clear our reference to the TimeoutFuture
         * is the reason we use a *static* nested class with a manual reference back to
         * the "containing" class.)
         *
         * This has the nice-ish side effect of limiting reentrancy: run() calls
         * timeoutFuture.setException() calls run(). That reentrancy would already be
         * harmless, since timeoutFuture can be set (and delegate cancelled) only once.
         * (And "set only once" is important for other reasons: run() can still be
         * invoked concurrently in different threads, even with the above null checks.)
         */
    ...
    void done() {
      ...
      // happens-before edge between the assignment to timer and an execution of the
      // timer task.
      if (localTimer != null) {
  ...
  /**
   * Returns a new {@code ListenableFuture} whose result is asynchronously derived
   * from the result of the given {@code Future}. More precisely, the returned
   * {@code Future} takes its result from a {@code Future} produced by applying
   * the given {@code AsyncFunction} to the result of the original {@code Future}.
   * Example:
   *
   * <pre>
   * {
   ...
   *
   * <p>
   * The returned {@code Future} attempts to keep its cancellation state in sync
   * with that of the input future and that of the future returned by the
   * function. That is, if the returned {@code Future} is cancelled, it will
   * attempt to cancel the other two, and if either of the other two is cancelled,
   * the returned {@code Future} will receive a callback in which it will attempt
   * to cancel itself.
   *
   ...
   *             overloads are keeping the "transform" name.) This method will be
   ...
   *     }
   *   };
   *   ListenableFuture<QueryResult> queryFuture = transform(rowKeyFuture, queryFunction, executor);
   * }
   * </pre>
   *
   * <p>
   ...
   * <p>
   * The returned {@code Future} attempts to keep its cancellation state in sync
   * with that of the input future and that of the future returned by the chain
   * function. That is, if the returned {@code Future} is cancelled, it will
   * attempt to cancel the other two, and if either of the other two is cancelled,
   * the returned {@code Future} will receive a callback in which it will attempt
   * to cancel itself.
   ...
   * The returned {@code Future} attempts to keep its cancellation state in sync
   * with that of the input future and that of the future returned by the
   * function. That is, if the returned {@code
   * Future} is cancelled, it will attempt to cancel the other two, and if either
   * of the other two is cancelled, the returned {@code Future} will receive a
  ...
  /**
   * Returns an Executor that will propagate {@link RejectedExecutionException}
   * from the delegate executor to the given {@code future}.
   *
   * <p>
   ...
   * <p>
   * The returned {@code Future} attempts to keep its cancellation state in sync
   * with that of the input future. That is, if the returned {@code Future} is
   * cancelled, it will attempt to cancel the input, and if the input is
   ...
   *                 the results of the returned future. This will be run in the
   *                 thread that notifies input it is complete.
   ...
   * @param input    The future to transform
   * @param function A Function to transform the results of the provided future to
   *                 the results of the returned future.
   * @param executor Executor to run the function in.
   * @return A future that holds result of the transformation.
   ...
   *
   * <p>
   * The returned {@code Future} reflects the input's cancellation state directly,
   * and any attempt to cancel the returned Future is likewise passed through to
   * the input Future.
   *
   * <p>
   * Note that calls to {@linkplain Future#get(long, TimeUnit) timed get} only
   * apply the timeout to the execution of the underlying {@code Future},
   ...
   * have a {@code ListenableFuture} available and do not mind repeated, lazy
   * function evaluation.
   *
   * @param input    The future to transform
   * @param function A Function to transform the results of the provided future to
   ...
   * {@code Runnable} so that it can be used to nest ListenableFutures. Once the
   * passed-in {@code ListenableFuture} is complete, it calls the passed-in
   * {@code Function} to generate the result.
   *
   * <p>
   ...
   * and uses its <i>cause</i> as the output future's exception, rather than using
   * the {@code UndeclaredThrowableException} itself as it would for other
   ...
   * throw checked exceptions. Nowadays, {@code Futures.transform} has an overload
   * that accepts an {@code AsyncFunction}, whose {@code apply} method <i>is</i>
   * allowed to throw checked exception. Users who wish to throw checked
   * exceptions should use that overload instead, and
   * <a href="http://code.google.com/p/guava-libraries/issues/detail?id=1548">we
   * should remove the {@code UndeclaredThrowableException} special case</a>.
   */
  private abstract static class AbstractChainingFuture<I, O, F> extends AbstractFuture.TrustedFuture<O>
      implements Runnable {
    // In theory, this field might not be visible to a cancel() call in certain
    // circumstances. For
  ...
  /**
   * A {@link AbstractChainingFuture} that delegates to an {@link AsyncFunction}
   * and {@link #setFuture(ListenableFuture)} to implement {@link #doTransform}.
   */
  ...
  /**
   * Returns a new {@code ListenableFuture} whose result is the product of calling
   * {@code get()} on the {@code Future} nested within the given {@code
   ...
   * {@link #transform(ListenableFuture, AsyncFunction)}, in that the returned
   * {@code
   * Future} attempts to keep its cancellation state in sync with both the input
   * {@code Future} and the nested {@code Future}. The transformation is very
   * lightweight and therefore takes place in the same thread (either the thread
   * that called {@code dereference}, or the thread in which the dereferenced
   * future completes).
   *
   ...
   *
   * <p>
   * Canceling this future will attempt to cancel all the component futures, and
   * if any of the provided futures fails or is canceled, this one is, too.
   *
  ...
  /**
   * Creates a new {@code ListenableFuture} whose result is set from the supplied
   * future when it completes. Cancelling the supplied future will also cancel the
   * returned future, but cancelling the returned future will have no effect on
   * the supplied future.
   *
   * @since 15.0
   */
   ...
   * same order as the input list, and if any of the provided futures fails or is
   * canceled, its corresponding position will contain {@code null} (which is
   * indistinguishable from the future having a successful value of {@code null}).
   *
   * <p>
  ...
  /**
   * Returns a list of delegate futures that correspond to the futures received in
   * the order that they complete. Delegate futures return the same value or throw
   * the same exception as the corresponding input future returns/throws.
   *
   * <p>
   * Cancelling a delegate future has no effect on any input future, since the
   * delegate future does not correspond to a specific input future until the
   * appropriate number of input futures have completed. At that point, it is too
   * late to cancel the input future. The input future's result, which cannot be
   * stored into the cancelled delegate future, is ignored.
   *
   * @since 17.0
   */
  @Beta
  ...
  public static <T> ImmutableList<ListenableFuture<T>> inCompletionOrder(
    ...
    // A CLQ may be overkill here. We could save some pointers/memory by
    // synchronizing on an
    // ArrayDeque
    final ConcurrentLinkedQueue<SettableFuture<T>> delegates = Queues.newConcurrentLinkedQueue();
    ImmutableList.Builder<ListenableFuture<T>> listBuilder = ImmutableList.builder();
    // Using SerializingExecutor here will ensure that each CompletionOrderListener
    // executes
    // atomically and therefore that each returned future is guaranteed to be in
    // completion order.
    // N.B. there are some cases where the use of this executor could have possibly
    // surprising
    // effects when input futures finish at approximately the same time _and_ the
    // output futures
    // have directExecutor listeners. In this situation, the listeners may end up
    // running on a
    // different thread than if they were attached to the corresponding input
    // future. We believe
    // this to be a negligible cost since:
    // 1. Using the directExecutor implies that your callback is safe to run on any
    // thread.
    // 2. This would likely only be noticeable if you were doing something expensive
    // or blocking on
    // a directExecutor listener on one of the output futures which is an
    // antipattern anyway.
    ...
    for (final ListenableFuture<? extends T> future : futures) {
      ...
      // Must make sure to add the delegate to the queue first in case the future is
      // already done
   ...
   *
   * <p>
   * There is no guaranteed ordering of execution of callbacks, but any callback
   * added through this method is guaranteed to be called once the computation is
   * complete.
   *
   * Example:
   * 
   ...
   *
   * <p>
   * The callback is run in {@code executor}. There is no guaranteed ordering of
   * execution of callbacks, but any callback added through this method is
   ...
   * {@code X} if the cause is a checked exception, an
   * {@link UncheckedExecutionException} if the cause is a {@code
   ...
   * {@code String}) and calling the constructor via reflection. If the exception
   * did not already have a cause, one is set by calling
   * {@link Throwable#initCause(Throwable)} on it. If no such constructor exists,
   * an {@code IllegalArgumentException} is thrown.
   *
   * @throws X                           if {@code get} throws any checked
   *                                     exception except for an {@code
   *         ExecutionException}      whose cause is not itself a checked
   *                                     exception
   * @throws UncheckedExecutionException if {@code get} throws an {@code
   *         ExecutionException}      with a {@code RuntimeException} as its
   *                                     cause
   * @throws ExecutionError              if {@code get} throws an
   *                                     {@code ExecutionException} with an
   ...
   *             the change in parameter order. This method will be removed in
   *             Guava release 20.0.
   */
  @Deprecated
  @GwtIncompatible("reflection")
  ...
  /**
   * Returns the result of calling {@link Future#get()} uninterruptibly on a task
   * known not to throw a checked exception. This makes {@code Future} more
   * suitable for lightweight, fast-running tasks that, barring bugs in the code,
   * will not fail. This gives it exception-handling behavior similar to that of
   * {@code ForkJoinTask.join}.
   *
   * <p>
   * Exceptions from {@code Future.get} are treated as follows:
   * <ul>
   ...
   *     Error}).
   * <li>Any {@link InterruptedException} causes a retry of the {@code get} call.
   * The interrupt is restored before {@code getUnchecked} returns.
   * <li>Any {@link CancellationException} is propagated untouched. So is any
   * other {@link RuntimeException} ({@code get} implementations are discouraged
  ...
  private static void wrapAndThrowUnchecked(Throwable cause) {
    ...
    /*
     * It's a non-Error, non-Exception Throwable. From my survey of such classes, I
     * believe that most users intended to extend Exception, so we'll treat it like
     * an Exception.
     */
  ...
  /*
   * Arguably we don't need a timed getUnchecked because any operation slow enough
   * to require a timeout is heavyweight enough to throw a checked exception and
   * therefore be inappropriate to use with getUnchecked. Further, it's not clear
   * that converting the checked TimeoutException to a RuntimeException --
   * especially to an UncheckedExecutionException, since it wasn't thrown by the
   * computation -- makes sense, and if we don't convert it, the user still has to
   * write a try-catch block.
   *
   * If you think you would use this method, let us know. You might also also look
