 ...
 *
 * <p>
 * It is possible to call {@link #addListener addListener} directly, but this is
 * uncommon because the {@code Runnable} interface does not provide direct
 ...
 * addListener} calls are occasionally useful:
 * 
 * <pre>
 *    {@code
 *   final String name = ...;
...
public interface ListenableFuture<V> extends Future<V> {
  /**
   * Registers a listener to be {@linkplain Executor#execute(Runnable) run} on the
   * given executor. The listener will run when the {@code Future}'s computation
   * is {@linkplain Future#isDone() complete} or, if the computation is already
   * complete, immediately.
   *
   * <p>
   * There is no guaranteed ordering of execution of listeners, but any listener
   ...
   * exception thrown during {@code Executor.execute} (e.g., a {@code
   * RejectedExecutionException} or an exception thrown by
   * {@linkplain MoreExecutors#directExecutor direct execution}) will be caught
   * and logged.
   *
   * <p>
   * Note: For fast, lightweight listeners that would be safe to execute in any
   * thread, consider {@link MoreExecutors#directExecutor}. Otherwise, avoid it.
   * Heavyweight {@code directExecutor} listeners can cause problems, and these
   * problems can be difficult to reproduce because they depend on timing. For
   * example:
   *
   * <ul>
   * <li>The listener may be executed by the caller of {@code addListener}. That
   * caller may be a UI thread or other latency-sensitive thread. This can harm UI
   * responsiveness.
   * <li>The listener may be executed by the thread that completes this {@code
   * Future}. That thread may be an internal system thread such as an RPC network
   * thread. Blocking that thread may stall progress of the whole system. It may
   * even cause a deadlock.
   * <li>The listener may delay other listeners, even listeners that are not
   * themselves {@code directExecutor} listeners.
   * </ul>
   *
   * <p>
   * This is the most general listener interface. For common operations performed
   * using listeners, see {@link Futures}. For a simplified but general listener
   * interface, see {@link Futures#addCallback addCallback()}.
   *
   * <p>
