...
import com.google.v19.common.base.Supplier;
...
import com.google.v19.common.util.concurrent.ForwardingListenableFuture.SimpleForwardingListenableFuture;
...
/**
 * Factory and utility methods for {@link java.util.concurrent.Executor},
 * {@link ExecutorService}, and {@link ThreadFactory}.
 *
...
public final class MoreExecutors {
  ...
  /**
   * Converts the given ThreadPoolExecutor into an ExecutorService that exits when
   * the application is complete. It does so by using daemon threads and adding a
   * shutdown hook to wait for their completion.
   *
   * <p>
   * This is mainly for fixed thread pools. See
   * {@link Executors#newFixedThreadPool(int)}.
   *
   * @param executor           the executor to modify to make sure it exits when
   ...
   * ScheduledExecutorService that exits when the application is complete. It does
   * so by using daemon threads and adding a shutdown hook to wait for their
   * completion.
   *
   * <p>
   * This is mainly for fixed thread pools. See
   ...
   * shutdown, instead giving these daemon threads a chance to terminate normally.
   * 
   ...
   *
   * <p>
   * This method waits 120 seconds before continuing with JVM termination, even if
   * the executor has not finished its work.
   *
   * <p>
   * This is mainly for fixed thread pools. See
   * {@link Executors#newFixedThreadPool(int)}.
   ...
   * {@code execute/submit}, as in {@link CallerRunsPolicy}. This applies both to
   * individually submitted tasks and to collections of tasks submitted via
   * {@code invokeAll} or {@code invokeAny}. In the latter case, tasks will run
   * serially on the calling thread. Tasks are run to completion before a
   * {@code Future} is returned to the caller (unless the executor has been
   * shutdown).
   *
   * <p>
   * Although all tasks are immediately executed in the thread that submitted the
   * task, this {@code ExecutorService} imposes a small locking overhead on each
   * task submission in order to implement shutdown and termination behavior.
   *
   * <p>
   * The implementation deviates from the {@code ExecutorService} specification
   * with regards to the {@code shutdownNow} method. First, "best-effort" with
   * regards to canceling running tasks is implemented as "no-effort". No
   * interrupts or other attempts are made to stop threads executing tasks.
   * Second, the returned list will always be empty, as any submitted task is
   * considered to have started execution. This applies also to tasks given to
   * {@code invokeAll} or {@code invokeAny} which are pending serial execution,
   * even the subset of the tasks that have not yet started execution. It is
   * unclear from the {@code ExecutorService} specification if these should be
   * included, and it's much easier to implement the interpretation that they not
   * be. Finally, a call to {@code shutdown} or {@code shutdownNow} may result in
   * concurrent calls to {@code invokeAll/invokeAny} throwing
   * RejectedExecutionException, although a subset of the tasks may already have
   * been executed.
   *
   * @since 10.0
   *        (<a href="https://github.com/google/guava/wiki/Compatibility" >mostly
   *        source-compatible</a> since 3.0)
   * @deprecated Use {@link #directExecutor()} if you only require an
   *             {@link Executor} and {@link #newDirectExecutorService()} if you
   *             need a {@link ListeningExecutorService}. This method will be
   ...
   *
   * <p>
   * This should be preferred to {@link #newDirectExecutorService()} because the
   * implementing the {@link ExecutorService} subinterface necessitates
   ...
   * invokeAll} methods submit {@link ListenableFutureTask} instances to the given
   * delegate executor. Those methods, as well as {@code execute} and
   * {@code invokeAny}, are implemented in terms of calls to {@code
   * delegate.execute}. All other methods are forwarded unchanged to the delegate.
   * This implies that the returned {@code ListeningExecutorService} never calls
   * the delegate's {@code submit}, {@code invokeAll}, and {@code
   ...
   * {@code invokeAny}, are implemented in terms of calls to {@code
   * delegate.execute}. All other methods are forwarded unchanged to the delegate.
   * This implies that the returned {@code
   ...
   * <p>
   * If the delegate executor was already an instance of {@code
   * ListeningScheduledExecutorService}, it is returned untouched, and the rest of
  ...
  /**
   * Submits the task and adds a listener that adds the future to {@code queue}
   * when it completes.
   */
  @GwtIncompatible("TODO")
  ...
  /**
   * Creates an {@link Executor} that renames the {@link Thread threads} that its
   * tasks run in.
   *
   * <p>
   * The names are retrieved from the {@code nameSupplier} on the thread that is
   * being renamed right before each task is run. The renaming is best effort, if
   * a {@link SecurityManager} prevents the renaming then it will be skipped but
   * the tasks will still execute.
   *
   *
  ...
  /**
   * Creates an {@link ExecutorService} that renames the {@link Thread threads}
  ...
  /**
   * Creates a {@link ScheduledExecutorService} that renames the {@link Thread
   * threads} that its tasks run in.
   *
   * <p>
   * The names are retrieved from the {@code nameSupplier} on the thread that is
   * being renamed right before each task is run. The renaming is best effort, if
  ...
  /**
   * Shuts down the given executor gradually, first disabling new submissions and
   * later cancelling existing tasks.
   *
   * <p>
   * The method takes the following steps:
   * <ol>
   * <li>calls {@link ExecutorService#shutdown()}, disabling acceptance of new
   * submitted tasks.
   ...
   *
   * <p>
   * If, at any step of the process, the calling thread is interrupted, the method
   * calls {@link ExecutorService#shutdownNow()} and returns.
   *
   * @param service the {@code ExecutorService} to shut down
   * @param timeout the maximum time to wait for the {@code ExecutorService} to
   *                terminate
   * @param unit    the time unit of the timeout argument
   * @return {@code true} if the {@code ExecutorService} was terminated
   *         successfully, {@code false} the call timed out or was interrupted
   * @since 17.0
   */
  @Beta
  @GwtIncompatible("concurrency")
