 ...
 * {@link Future} instances. Note that, whenver possible, it is strongly
 * preferred to modify those libraries to return {@code ListenableFuture}
 * directly.
 *
 * @author Sven Mawson
...
public final class JdkFutureAdapters {
   ...
   *
   * <p>
   * <b>Warning:</b> If the input future does not already implement {@code
   * ListenableFuture}, the returned future will emulate
   * {@link ListenableFuture#addListener} by taking a thread from an internal,
   * unbounded pool at the first call to {@code addListener} and holding it until
   * the future is {@linkplain Future#isDone() done}.
   *
   * <p>
   ...
   * executor at the first call to {@code addListener}. The task must be started
   * by the executor promptly, or else the returned {@code ListenableFuture} may
   * fail to work. The task's execution consists of blocking until the input
   * future is {@linkplain Future#isDone() done}, so each call to this method may
   ...
   * will wait on the future to finish, and when it completes, run the listeners.
   * This implementation will wait on the source future indefinitely, so if the
   * source future never completes, the adapter will never complete either.
   *
   * <p>
        ...
        adapterExecutor.execute(new Runnable() {
          ...
          public void run() {
            try {
               ...
               * user-supplied executor might be, but... what can we do? This is another
               * reason to return a proper ListenableFuture instead of using
               * listenInPoolThread.
               */
