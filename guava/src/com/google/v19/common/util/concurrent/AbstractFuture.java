...
/**
 * An abstract implementation of {@link ListenableFuture}, intended for advanced
 * users only. More common ways to create a {@code ListenableFuture} include
 * instantiating a {@link SettableFuture}, submitting a task to a
 * {@link ListeningExecutorService}, and deriving a {@code Future} from an
 * existing one, typically using methods like
 * {@link Futures#transform(ListenableFuture, Function) Futures.transform} and
 * {@link Futures#catching(ListenableFuture, Class, Function) Futures.catching}.
 *
 * <p>
 * This class implements all methods in {@code ListenableFuture}. Subclasses
 * should provide a way to set the result of the computation through the
 * protected methods {@link #set(Object)}, {@link #setFuture(ListenableFuture)}
 * and {@link #setException(Throwable)}. Subclasses may also override
 * {@link #interruptTask()}, which will be invoked automatically if a call to
 * {@link #cancel(boolean) cancel(true)} succeeds in canceling the future.
 * Subclasses should rarely override other methods.
 *
 * @author Sven Mawson
...
public abstract class AbstractFuture<V> implements ListenableFuture<V> {
   ...
   * setFuture by ensuring that {@link #get} calls exactly the implementation of
   * {@link AbstractFuture#get}.
   */
  abstract static class TrustedFuture<V> extends AbstractFuture<V> {
    // N.B. cancel is not overridden to be final, because many future utilities need
    // to override
    // cancel in order to propagate cancellation to other futures.
    // TODO(lukes): with maybePropagateCancellation this is no longer really true.
    // Track down the
    ...
    try {
      ...
      try {
        helper = new SafeAtomicHelper(newUpdater(Waiter.class, Thread.class, "thread"),
            newUpdater(Waiter.class, Waiter.class, "next"), newUpdater(AbstractFuture.class, Waiter.class, "waiters"),
  ...
  private static final class Waiter {
    ...
    // is loaded
    // before the ATOMIC_HELPER. Apparently this is possible on some android
    // platforms.
    ...
    // non-volatile write to the next field. Should be made visible by subsequent
    // CAS on waiters
    // field.
    ...
    void unpark() {
      ...
      // spuriously
      // call unpark even though the thread has already removed itself from the list.
      // But even if
      // we did use a CAS, that race would still exist (it would just be ever so
      // slightly smaller).
      Thread w = thread;
      if (w != null) {
        thread = null;
  ...
  /**
   * Marks the given node as 'deleted' (null waiter) and then scans the list to
   * unlink all deleted nodes. This is an O(n) operation in the common case (and
   * O(n^2) in the worst), but we are saved by two things.
   * <ul>
   * <li>This is only called when a waiting thread times out or is interrupted.
   * Both of which should be rare.
  ...
  // TODO(lukes): investigate using the @Contended annotation on these fields when
  // jdk8 is
  // available.
  /**
  ...
  // latency overheads (I
  // have observed 12 micros on 64 bit linux systems to wake up a parked thread).
  // So if the
  // timeout is small we shouldn't park(). This needs to be traded off with the
  // cpu overhead of
  ...
  // parkNanos depends on
  // system scheduling and as such we could either miss our deadline, or unpark()
  // could be delayed
  // so that it looks like we timed out even though we didn't. For comparison
  // FutureTask respects
  // completion preferably and AQS is non-deterministic (depends on where in the
  // queue the waiter
  // is). If we wanted to be strict about it, we could store the unpark() time in
  // the Waiter
  // node and we could use that to make a decision about whether or not we timed
  // out prior to
  // being unparked.
   ...
   *                               during the call (optional but recommended).
  ...
  public V get(long timeout, TimeUnit unit) throws InterruptedException, TimeoutException, ExecutionException {
        ...
        do {
          ...
          if (ATOMIC_HELPER.casWaiters(this, oldHead, node)) {
            while (true) {
              ...
              // Check interruption first, if we woke up due to interruption we need to honor
              // that.
              if (Thread.interrupted()) {
                removeWaiter(node);
                throw new InterruptedException();
    ...
    // If we get here then we have remainingNanos < SPIN_THRESHOLD_NANOS and there
    // is no node on the
    // waiters list
   ...
   *
   * <p>
   * If a cancellation attempt succeeds on a {@code Future} that had previously
   * been {@linkplain #setFuture set asynchronously}, then the cancellation will
   * also be propagated to the delegate {@code Future} that was supplied in the
   * {@code setFuture} call.
   */
  @Override
  public boolean cancel(boolean mayInterruptIfRunning) {
    ...
    if (localValue == null | localValue instanceof AbstractFuture.SetFuture) {
      ...
      // TODO(lukes): this exception actually makes cancellation significantly more
      // expensive :(
      // I wonder if we should consider removing it or providing a mechanism to not do
      // it.
      ...
      do {
        if (ATOMIC_HELPER.casValue(this, localValue, valueToSet)) {
          // We call interuptTask before calling complete(), first which is consistent
          // with
          // FutureTask
          ...
          if (localValue instanceof AbstractFuture.SetFuture) {
            // propagate cancellation to the future set in setfuture, this is racy, and we
            // don't
  ...
  /**
   * Returns an exception to be used as the cause of the CancellationException
   * thrown by {@link #get}.
   *
   * <p>
   * Note: this method may be called speculatively. There is no guarantee that the
   * future will be cancelled if this method is called.
   */
  ...
  /**
   * Sets the result of this {@code Future} unless this {@code Future} has already
   * been cancelled or set (including {@linkplain #setFuture set asynchronously}).
   * When a call to this method returns, the {@code Future} is guaranteed to be
   * {@linkplain #isDone done} <b>only if</b> the call was accepted (in which case
   * it returns {@code true}). If it returns {@code false}, the {@code
   * Future} may have previously been set asynchronously, in which case its result
   * may not be known yet. That result, though not yet known, cannot by overridden
   * by a call to a {@code set*} method, only by a call to {@link #cancel}.
   *
   ...
   * guaranteed to be {@linkplain #isDone done} <b>only if</b> the call was
   * accepted (in which case it returns {@code true}). If it returns
   * {@code false}, the {@code Future} may have previously been set
   * asynchronously, in which case its result may not be known yet. That result,
   * though not yet known, cannot by overridden by a call to a {@code set*}
   ...
   *
   * <p>
   * If the supplied future is {@linkplain #isDone done} when this method is
   * called and the call is accepted, then this future is guaranteed to have been
   * completed with the supplied future by the time this method returns. If the
   * supplied future is not done and the call is accepted, then the future will be
   * <i>set asynchronously</i>. Note that such a result, though not yet known,
   * cannot by overridden by a call to a {@code set*} method, only by a call to
   * {@link #cancel}.
   *
   * <p>
   * If the call {@code setFuture(delegate)} is accepted and this {@code Future}
   * is later cancelled, cancellation will be propagated to {@code delegate}.
   * Additionally, any call to {@code setFuture} after any cancellation will
   * propagate cancellation to the supplied {@code
   * Future}.
   *
   * @param future the future to delegate to
   * @return true if the attempt was accepted, indicating that the {@code Future}
  ...
  private boolean completeWithFuture(ListenableFuture<? extends V> future, Object expected) {
    ...
    if (future instanceof TrustedFuture) {
      // Break encapsulation for TrustedFuture instances since we know that subclasses
      // cannot
      // override .get() (since it is final) and therefore this is equivalent to
      // calling .get()
      // and unpacking the exceptions like we do below (just much faster because it is
      // a single
    ...
    // The only way this can fail is if we raced with another thread calling
    // cancel(). If we lost
  ...
  private void complete() {
    ...
    // We call this after the listeners on the theory that done() will only be used
    // for 'cleanup'
    // oriented tasks (e.g. clearing fields) and so can wait behind listeners which
    // may be executing
    // more important work. A counter argument would be that done() is trusted code
    // and therefore
    // it would be safe to run before potentially slow or poorly behaved listeners.
    // Reevaluate this
   ...
   *
   * <p>
   * This is called exactly once, after all listeners have executed. By default it
   * does nothing.
   */
  void done() {
  }
  ...
  /**
   * Returns the exception that this {@code Future} completed with. This includes
   * completion through a call to {@link setException} or
   * {@link setFuture}{@code (failedFuture)} but not cancellation.
   *
  ...
  /**
   * If this future has been cancelled (and possibly interrupted), cancels (and
   * possibly interrupts) the given future (if available).
   *
   * <p>
   * This method should be used only when this future is completed. It is designed
   * to be called from {@code done}.
   */
  ...
  /**
   * Submits the given runnable to the given {@link Executor} catching and logging
   * all {@linkplain RuntimeException runtime exceptions} thrown by the executor.
   */
   ...
   *
   * <p>
   * Static initialization of this class will fail if the {@link sun.misc.Unsafe}
   ...
   *
   * <p>
   * This is an implementation of last resort for when certain basic VM features
   * are broken (like AtomicReferenceFieldUpdater).
   */
