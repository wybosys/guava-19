...
/**
 * Produces proxies that impose a time limit on method calls to the proxied
 * object. For example, to return the value of {@code target.someMethod()}, but
 * substitute {@code DEFAULT_VALUE} if this method call takes over 50 ms, you
 * can use this code:
 * 
 * <pre>
 *   TimeLimiter limiter = . . .;
...
public interface TimeLimiter {
    ...
    /**
     * Returns an instance of {@code interfaceType} that delegates all method calls
     * to the {@code target} object, enforcing the specified time limit on each
     * call. This time-limited delegation is also performed for calls to
     ...
     * method call is of secondary concern. The particular nature and strength of
     * the guarantees made by the proxy is implementation-dependent. However, it is
     * important that each of the methods on the target object behaves appropriately
     ...
     *                        callers are willing to wait on each method call to the
     *                        proxy
     * @param timeoutUnit     with timeoutDuration, the maximum length of time that
     *                        callers are willing to wait on each method call to the
     *                        proxy
     * @return a time-limiting proxy
     * @throws IllegalArgumentException if {@code interfaceType} is a regular class,
     *                                  enum, or annotation type, rather than an
     *                                  interface
     */
