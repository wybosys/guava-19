...
import static com.google.v19.common.io.ByteStreams.BUF_SIZE;
import static com.google.v19.common.io.ByteStreams.skipUpTo;
...
public abstract class ByteSource {
   ...
   * bytes long starting at the given {@code offset}. If {@code offset} is greater
   * than the size of this source, the returned source will be empty. If
   * {@code offset + length} is greater than the size of this source, the returned
   * source will contain the slice starting at {@code offset} and ending at the
   * end of this source.
   *
  ...
  /**
   * Returns whether the source has zero bytes. The default implementation returns
   * true if {@link #sizeIfKnown} returns zero, falling back to opening a stream
   * and checking for EOF if the size is not known.
   *
   * <p>
   * Note that, in cases where {@code sizeIfKnown} returns zero, it is
   * <i>possible</i> that bytes are actually available for reading. (For example,
   * some special files may return a size of 0 despite actually having content
   * when read.) This means that a source may return {@code true} from
   ...
   * such as a file, may return a non-absent value. Note that in such cases, it is
   * <i>possible</i> that this method will return a different number of bytes than
   * would be returned by reading all of the bytes (for example, some special
   * files may return a size of 0 despite actually having content when read).
   *
   * <p>
   * Additionally, for mutable sources such as files, a subsequent read may return
   * a different number of bytes if the contents are changed.
   *
   * @since 19.0
   */
  @Beta
  ...
  /**
   * Returns the size of this source in bytes, even if doing so requires opening
   * and traversing an entire stream. To avoid a potentially expensive operation,
   * see {@link #sizeIfKnown}.
   *
   * <p>
   * The default implementation calls {@link #sizeIfKnown} and returns the value
   * if present. If absent, it will fall back to a heavyweight operation that will
   * open a stream, read (or {@link InputStream#skip(long) skip}, if possible) to
   * the end of the stream and return the total number of bytes that were read.
   *
   * <p>
   * Note that for some sources that implement {@link #sizeIfKnown} to provide a
   * more efficient implementation, it is <i>possible</i> that this method will
   * return a different number of bytes than would be returned by reading all of
   * the bytes (for example, some special files may return a size of 0 despite
   * actually having content when read).
   *
   * <p>
