...
import static com.google.v19.common.base.Preconditions.checkPositionIndex;
...
public final class ByteStreams {
  ...
  /**
   * A buffer for skipping bytes in an input stream. Only written to and never
   * read, so actual contents don't matter.
   */
   ...
   *
   * <ol>
   * <li>Use sendfile(2) or equivalent. Requires that both the input channel and
   * the output channel have their own file descriptors. Generally this only
   * happens when both channels are files or sockets. This performs zero copies -
   * the bytes never enter userspace.</li>
   * <li>Use mmap(2) or equivalent. Requires that either the input channel or the
   * output channel have file descriptors. Bytes are copied from the file into a
   * kernel buffer, then directly into the other buffer (userspace). Note that if
   * the file is very large, a naive implementation will effectively put the whole
   * file in memory. On many systems with paging and virtual memory, this is not a
   * problem - because it is mapped read-only, the kernel can always page it to
   * disk "for free". However, on systems where killing processes happens all the
   * time in normal conditions (i.e., android) the OS must make a tradeoff between
   * paging memory and killing other processes - so allocating a gigantic buffer
   * and then sequentially accessing it could result in other processes dying.
   * This is solvable via madvise(2), but that obviously doesn't exist in
   * java.</li>
   * <li>Ordinary copy. Kernel copies bytes into a kernel buffer, from a kernel
   * buffer into a userspace buffer (byte[] or ByteBuffer), then copies them from
   * that buffer into the destination channel.</li>
   * </ol>
   *
   * This value is intended to be large enough to make the overhead of system
   * calls negligible, without being so large that it causes problems for systems
   * with atypical memory management if approaches 2 or 3 are used.
   */
   ...
   *
   * @since 14.0 (since 1.0 as com.google.v19.common.io.NullOutputStream)
   */
   ...
   * @since 14.0 (since 1.0 as com.google.v19.common.io.LimitInputStream)
   */
   ...
   * will block until either the full amount has been skipped or until the end of
   * the stream is reached, whichever happens first. Returns the total number of
   * bytes skipped.
   */
  ...
  /**
   * Attempts to skip up to {@code n} bytes from the given input stream, but not
   * more than {@code in.available()} bytes. This prevents {@code FileInputStream}
   * from skipping more bytes than actually remain in the file, something that it
   * {@linkplain FileInputStream#skip(long) specifies} it can do in its Javadoc
   * despite the fact that it is violating the contract of
