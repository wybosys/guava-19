...
import com.google.v19.common.collect.AbstractIterator;
...
public abstract class CharSource {
   ...
   * such as a {@code CharSequence}, may return a non-absent value. Note that in
   * such cases, it is <i>possible</i> that this method will return a different
   * number of chars than would be returned by reading all of the chars.
   *
   * <p>
   * Additionally, for mutable sources such as {@code StringBuilder}s, a
   * subsequent read may return a different number of chars if the contents are
   * changed.
   *
   * @since 19.0
   */
  ...
  /**
   * Returns the length of this source in chars, even if doing so requires opening
   * and traversing an entire stream. To avoid a potentially expensive operation,
   * see {@link #lengthIfKnown}.
   *
   * <p>
   * The default implementation calls {@link #lengthIfKnown} and returns the value
   * if present. If absent, it will fall back to a heavyweight operation that will
   * open a stream, {@link Reader#skip(long) skip} to the end of the stream, and
   * return the total number of chars that were skipped.
   *
   * <p>
   * Note that for sources that implement {@link #lengthIfKnown} to provide a more
   * efficient implementation, it is <i>possible</i> that this method will return
   * a different number of chars than would be returned by reading all of the
   * chars.
   *
   * <p>
   ...
   *
   * @throws IOException if an I/O error occurs in the process of reading the
  ...
  /**
   * Returns whether the source has zero chars. The default implementation returns
   * true if {@link #lengthIfKnown} returns zero, falling back to opening a stream
   * and checking for EOF if the length is not known.
   *
   * <p>
   ...
   * that a source may return {@code true} from {@code isEmpty()} despite having
   * readable content.
   *
